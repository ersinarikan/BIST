...
class AsyncVisualPatternSystem:
    ...
    def __init__(self, worker_threads: int = 2):
        ...
        try:
            self.worker_threads = int(os.getenv('YOLO_WORKER_THREADS', str(worker_threads)))
        except Exception:
            self.worker_threads = worker_threads
        
        # Model state
        self._model = None
        self._model_path = os.getenv('YOLO_MODEL_PATH', '/opt/bist-pattern/yolo/patterns_all_v7_rectblend.pt')
        ...
        # Background workers
        self._loader_pool = ThreadPoolExecutor(max_workers=1, thread_name_prefix="YOLOLoader")
        self._prediction_pool = ThreadPoolExecutor(max_workers=self.worker_threads, thread_name_prefix="YOLOPredict")
        ...
        logger.info(f"🎯 Async Visual Pattern System initialized (enabled: {self._enable_yolo}, available: {self.yolo_available})")
    ...
    def _try_import_backend(self):
        ...
        try:
            ...
            logger.info("📊 YOLO backend not available (ultralytics not installed)")
        ...
        def load_model_async():
            try:
                with self._model_load_lock:
                    ...
                    logger.info(f"🔄 Background loading YOLO model: {self._model_path}")
                
                # This is the blocking operation, but in background thread
                if not os.path.exists(self._model_path):
                    logger.warning(f"YOLO model file not found: {self._model_path}")
                    with self._model_load_lock:
                        self._model_loading = False
                    return
                ...
                logger.info(f"✅ YOLO model loaded successfully: {self._model_path}")
    ...
    def _canonicalize_class_name(self, raw_name: str) -> str:
        ...
        Returns UPPER_SNAKE_CASE canonical name when possible; otherwise sanitized upper name.
        """
        try:
    ...
    def request_visual_analysis_async(self, symbol: str, stock_data: Any) -> str:
        """
        ...
        with self._model_load_lock:
            if self._model_loaded and self._model is not None:
                ...
                self._prediction_pool.submit(self._process_visual_prediction, request_id, symbol, stock_data)
    ...
    def _process_visual_prediction(self, request_id: str, symbol: str, stock_data: Any):
        ...
        try:
            ...
            if results and len(results) > 0:
                ...
                if boxes is not None and hasattr(boxes, 'cls') and hasattr(boxes, 'conf'):
                    # Determine number of detections robustly across ultralytics versions
                    try:
                    ...
                    for i in range(num):
                        try:
                            ...
                            cls_idx = int(boxes.cls[i]) if hasattr(boxes, 'cls') else 0
            ...
            try:
                if patterns:
                    ...
                    logger.info(f"📸 YOLO visual detections for {symbol}: {len(patterns)}")
            except Exception:
                pass
            ...
            logger.debug(f"✅ YOLO visual analysis completed for {symbol}: {len(patterns)} patterns")
    ...
    def _render_chart_image(self, stock_data):
        ...
        try:
            ...
            if 'volume' in stock_data.columns:
                ax2 = ax.twinx()
                ax2.bar(range(len(stock_data)), stock_data['volume'], alpha=0.3, color='gray')
    ...
    def get_visual_analysis_result(self, request_id: str) -> Optional[Dict[str, Any]]:
    ...
    def get_system_info(self) -> Dict[str, Any]:
        ...
        with self._model_load_lock:
            return {
                ...
                'queue_size': self._prediction_queue.qsize() if hasattr(self._prediction_queue, 'qsize') else 0
            }
    ...
    def cleanup_old_results(self, max_age_seconds: int = 300):
        ...
        with self._prediction_lock:
            to_remove = [
                req_id for req_id, result in self._prediction_results.items()
        ...
        if to_remove:
            logger.debug(f"🧹 Cleaned up {len(to_remove)} old visual analysis results")
