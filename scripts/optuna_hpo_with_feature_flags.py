...
Bu script, hem hyperparameter'ları hem de feature flag'leri birlikte
optimize eder.
Optuna'nın suggest_categorical() fonksiyonu ile feature flag'leri de
optimize edebiliriz.

Örnek kullanım:
    python scripts/optuna_hpo_with_feature_flags.py --symbols ASELS
    --horizon 7 --trials 200
"""
...
def fetch_prices(
    engine, symbol: str, limit: int = 1200
) -> pd.DataFrame | None:
    """Fetch stock prices - ⚡ CRITICAL: Ensure we get enough data for HPO
    (at least 200 days).
    ...
    # Cache might be stale or incomplete (e.g., A1YEN has 130 days in cache
    # but 139 in DB)
    ...
    # ✅ Ensure datetime index (prevents macro merge fallback and TZ
    # mismatches)
    try:
        df.index = pd.to_datetime(df.index).tz_localize(None)
    except Exception as e:
        logger.debug(
            f"Failed to parse datetime index, trying coerce: {e}"
        )
        try:
            ...
            logger.warning(
                f"Failed to parse datetime index even with coerce: {e2}"
            )
    df = df.sort_index()

    # ✅ HİBRİT YAKLAŞIM: Duplicate date kontrolü (aynı tarihli kayıtlar
    # varsa temizle)
    if df.index.duplicated().any():
        duplicate_count = df.index.duplicated().sum()
        print(
            f"[hpo] {symbol}: {duplicate_count} duplicate date found, "
            f"dropping (keep='last')",
            file=sys.stderr,
            flush=True
        )
...
def _extract_model_metrics_for_horizon(
    train_result: Any, horizon: int
) -> Dict[str, Dict[str, float]]:
    """
    Pull lightweight metrics (raw_r2, rmse, mape, confidence) for each base
    model
...
def generate_walkforward_splits(
    total_days: int, horizon: int, n_splits: int = 4
) -> list[tuple[int, int]]:
    ...
    # Use approximately 30 days per split, but ensure we have enough test
    # data
    # ✅ FIX: Changed from * n_splits to * 2 for more flexible split
    # generation
    # This allows more symbols to have 2+ splits (enabling DirHit
    # calculation)
    # while maintaining statistical reliability (min 2 splits required)
    available_test_days = total_days - initial_split
    # Changed from * n_splits to * 2
    if available_test_days < min_test_days * 2:
...
def objective(
    ...
    """Optuna objective function - Feature flags + Hyperparameters birlikte
    optimize edilir."""
    # ✅ FIX: Check trial limit at the start of each trial to prevent
    # exceeding n_trials
    # This provides a second layer of protection against race conditions in
    # parallel execution
    ...
    if study is not None and max_trials is not None:
        ...
        other_trials = [
            t for t in study.trials if t.number != trial.number
        ]
        if len(other_trials) >= max_trials:
            # Skip this trial if we've already reached the limit
            raise optuna.TrialPruned(
                f"Trial limit reached ({len(other_trials)}/{max_trials})"
            )

    ConfigManager.clear_cache()

    # ⚡ NEW: Feature flag'leri optimize et (12 feature - test script ile
    # aynı)
    feature_flags = {
        'enable_external_features': trial.suggest_categorical(
            'enable_external_features', [True, False]
        ),
        'enable_fingpt_features': trial.suggest_categorical(
            'enable_fingpt_features', [True, False]
        ),
        'enable_yolo_features': trial.suggest_categorical(
            'enable_yolo_features', [True, False]
        ),
        'enable_directional_loss': trial.suggest_categorical(
            'enable_directional_loss', [True, False]
        ),
        'enable_seed_bagging': trial.suggest_categorical(
            'enable_seed_bagging', [True, False]
        ),
        'enable_talib_patterns': trial.suggest_categorical(
            'enable_talib_patterns', [True, False]
        ),
        'enable_smart_ensemble': trial.suggest_categorical(
            'enable_smart_ensemble', [True, False]
        ),
        'enable_stacked_short': trial.suggest_categorical(
            'enable_stacked_short', [True, False]
        ),
        'enable_meta_stacking': trial.suggest_categorical(
            'enable_meta_stacking', [True, False]
        ),
        'enable_regime_detection': trial.suggest_categorical(
            'enable_regime_detection', [True, False]
        ),
        'enable_fingpt': trial.suggest_categorical(
            'enable_fingpt', [True, False]
        ),
        # ML_USE_ADAPTIVE_LEARNING: HPO'da her zaman kapalı (data leakage
        # önleme)
    }

    # ⚡ NEW: Algorithm choice (controls which base learners are active)
    # ✅ CRITICAL FIX: Only suggest available models
    # This prevents Optuna from suggesting unavailable models (e.g.,
    # XGBoost if not installed)
    available_choices = []
    ...
    # Only add 'all' if multiple models are available
    if len(available_choices) > 1:
        available_choices.append('all')

    # ✅ DEBUG: Allow forcing a specific model via environment (e.g.,
    # FORCE_MODEL_CHOICE=xgb)
    # Useful for investigating model-specific issues without changing code
    # logic
    ...
    # Fallback: if no models available, use lgbm as default (should not
    # happen)
    if not available_choices:
        available_choices = ['lgbm']

    model_choice = trial.suggest_categorical(
        'model_choice', available_choices
    )
    enable_xgb = (
        (model_choice in ('xgb', 'all')) and XGBOOST_AVAILABLE
    )
    enable_lgb = (
        (model_choice in ('lgbm', 'all')) and LIGHTGBM_AVAILABLE
    )
    enable_cat = (
        (model_choice in ('cat', 'all')) and CATBOOST_AVAILABLE
    )

    # ⚡ NEW: Feature iç parametrelerini optimize et
    feature_params = {}

    # Directional Loss parametreleri (sadece enable_directional_loss=True
    # ise optimize et)
    if feature_flags['enable_directional_loss']:
        # MSE vs Directional balance
        feature_params['ml_loss_mse_weight'] = trial.suggest_float(
            'ml_loss_mse_weight', 0.2, 0.8
        )
        # Flat threshold
        feature_params['ml_loss_threshold'] = trial.suggest_float(
            'ml_loss_threshold', 0.001, 0.01, log=True
        )
        # Wrong direction penalty
        feature_params['ml_dir_penalty'] = trial.suggest_float(
            'ml_dir_penalty', 1.0, 5.0
        )

    # Seed Bagging parametreleri (sadece enable_seed_bagging=True ise
    # optimize et)
    if feature_flags['enable_seed_bagging']:
        # Number of seeds
        feature_params['n_seeds'] = trial.suggest_int('n_seeds', 2, 5)

    # Meta Stacking parametreleri (sadece enable_meta_stacking=True ise
    # optimize et)
    if feature_flags['enable_meta_stacking']:
        # Ridge alpha
        feature_params['meta_stacking_alpha'] = trial.suggest_float(
            'meta_stacking_alpha', 0.01, 10.0, log=True
        )

    # Adaptive Learning parametreleri (her zaman optimize et, çünkü Phase
    # 2 skip ediliyor ama Phase 1'de kullanılıyor)
    horizon_key = f'{horizon}d'
    # Adaptive K multiplier
    feature_params[f'ml_adaptive_k_{horizon_key}'] = trial.suggest_float(
        f'ml_adaptive_k_{horizon_key}', 1.0, 3.0
    )
    # Pattern weight scale
    feature_params[
        f'ml_pattern_weight_scale_{horizon_key}'
    ] = trial.suggest_float(
        f'ml_pattern_weight_scale_{horizon_key}', 0.5, 2.0
    )

    # TALIB (short horizons enable) - optional flag
    feature_flags['enable_talib_patterns_short'] = (
        trial.suggest_categorical(
            'enable_talib_patterns_short', [True, False]
        )
    )

    # External features internal params
    feature_params['external_min_days'] = trial.suggest_int(
        'external_min_days', 20, 60
    )
    feature_params['external_smooth_alpha'] = trial.suggest_float(
        'external_smooth_alpha', 0.05, 0.5
    )

    # Regime detection internal params (only if enabled)
    if feature_flags['enable_regime_detection']:
        feature_params['regime_scale_low'] = trial.suggest_float(
            'regime_scale_low', 0.80, 0.98
        )
        feature_params['regime_scale_high'] = trial.suggest_float(
            'regime_scale_high', 1.02, 1.20
        )

    # YOLO parametreleri (sadece enable_yolo_features=True ise optimize
    # et)
    if feature_flags['enable_yolo_features']:
        # YOLO confidence threshold
        feature_params['yolo_min_conf'] = trial.suggest_float(
            'yolo_min_conf', 0.3, 0.8
        )

    # Smart Ensemble parametreleri (sadece enable_smart_ensemble=True ise
    # optimize et)
    if feature_flags['enable_smart_ensemble']:
        # Consensus vs Performance balance
        feature_params['smart_consensus_weight'] = trial.suggest_float(
            'smart_consensus_weight', 0.3, 0.8
        )
        # Performance weight (complement of consensus)
        feature_params['smart_performance_weight'] = trial.suggest_float(
            'smart_performance_weight', 0.2, 0.7
        )
        # Sensitivity threshold
        feature_params['smart_sigma'] = trial.suggest_float(
            'smart_sigma', 0.001, 0.02, log=True
        )
        # ✅ NEW: Fixed per-model prior weights (applied in smart ensemble)
        feature_params['smart_weight_xgb'] = trial.suggest_float(
            'smart_weight_xgb', 0.5, 1.5
        )
        feature_params['smart_weight_lgbm'] = trial.suggest_float(
            'smart_weight_lgbm', 0.5, 1.5
        )
        feature_params['smart_weight_cat'] = trial.suggest_float(
            'smart_weight_cat', 0.5, 1.5
        )

    # FinGPT parametreleri (sadece enable_fingpt_features=True ise optimize
    # et)
    if feature_flags['enable_fingpt_features']:
        # Sentiment confidence threshold
        feature_params['fingpt_confidence_threshold'] = (
            trial.suggest_float(
                'fingpt_confidence_threshold', 0.2, 0.5
            )
        )

    # Feature flag'leri environment variable'lara set et (12 feature)
    os.environ['ENABLE_EXTERNAL_FEATURES'] = (
        '1' if feature_flags['enable_external_features'] else '0'
    )
    os.environ['ENABLE_FINGPT_FEATURES'] = (
        '1' if feature_flags['enable_fingpt_features'] else '0'
    )
    os.environ['ENABLE_YOLO_FEATURES'] = (
        '1' if feature_flags['enable_yolo_features'] else '0'
    )
    os.environ['ML_USE_DIRECTIONAL_LOSS'] = (
        '1' if feature_flags['enable_directional_loss'] else '0'
    )
    os.environ['ENABLE_SEED_BAGGING'] = (
        '1' if feature_flags['enable_seed_bagging'] else '0'
    )
    os.environ['ENABLE_TALIB_PATTERNS'] = (
        '1' if feature_flags['enable_talib_patterns'] else '0'
    )
    os.environ['ML_USE_SMART_ENSEMBLE'] = (
        '1' if feature_flags['enable_smart_ensemble'] else '0'
    )
    os.environ['ML_USE_STACKED_SHORT'] = (
        '1' if feature_flags['enable_stacked_short'] else '0'
    )
    os.environ['ENABLE_META_STACKING'] = (
        '1' if feature_flags['enable_meta_stacking'] else '0'
    )
    os.environ['ML_USE_REGIME_DETECTION'] = (
        '1' if feature_flags['enable_regime_detection'] else '0'
    )
    os.environ['ENABLE_FINGPT'] = (
        '1' if feature_flags['enable_fingpt'] else '0'
    )
    # TALIB short horizons
    os.environ['ENABLE_TALIB_PATTERNS_SHORT'] = (
        '1' if feature_flags.get('enable_talib_patterns_short') else '0'
    )
    ...
    os.environ[
        f'ML_ADAPTIVE_K_{horizon_key.upper()}'
    ] = str(feature_params[f'ml_adaptive_k_{horizon_key}'])
    os.environ[
        f'ML_PATTERN_WEIGHT_SCALE_{horizon_key.upper()}'
    ] = str(
        feature_params[f'ml_pattern_weight_scale_{horizon_key}']
    )
    ...
    os.environ['EXTERNAL_MIN_DAYS'] = str(
        feature_params['external_min_days']
    )
    os.environ['EXTERNAL_SMOOTH_ALPHA'] = str(
        feature_params['external_smooth_alpha']
    )

    # Regime params
    if feature_flags['enable_regime_detection']:
        os.environ['REGIME_SCALE_LOW'] = str(
            feature_params['regime_scale_low']
        )
        os.environ['REGIME_SCALE_HIGH'] = str(
            feature_params['regime_scale_high']
        )

    # ⚡ CRITICAL: Disable adaptive learning during HPO (data leakage
    # önleme)
        ...
        if horizon in (1, 3):
            params_xgb = {
                ...
                'subsample': trial.suggest_float(
                    'xgb_subsample', 0.5, 0.9
                ),
                'colsample_bytree': trial.suggest_float(
                    'xgb_colsample_bytree', 0.5, 0.9
                ),
                'reg_alpha': trial.suggest_float(
                    'xgb_reg_alpha', 1e-6, 1.0, log=True
                ),
                'reg_lambda': trial.suggest_float(
                    'xgb_reg_lambda', 1e-3, 50.0, log=True
                ),
                'min_child_weight': trial.suggest_int(
                    'xgb_min_child_weight', 5, 15
                ),
                'gamma': trial.suggest_float(
                    'xgb_gamma', 1e-4, 1.0, log=True
                ),
                # Advanced
                ...
                'tree_method': trial.suggest_categorical(
                    ...
                    (
                        ['hist', 'gpu_hist']
                        if str(os.getenv('XGB_ENABLE_GPU', '0')).lower()
                        in ('1', 'true', 'yes', 'on')
                        else ['hist']
                    )
                ),
            ...
            params_xgb = {
                'n_estimators': trial.suggest_int(
                    'xgb_n_estimators', 150, 900
                ),
                'max_depth': trial.suggest_int('xgb_max_depth', 3, 8),
                'learning_rate': trial.suggest_float(
                    'xgb_learning_rate', 0.01, 0.25, log=True
                ),
                'subsample': trial.suggest_float(
                ...
                'reg_alpha': trial.suggest_float(
                    'xgb_reg_alpha', 1e-6, 5.0, log=True
                ),
                'reg_lambda': trial.suggest_float(
                    'xgb_reg_lambda', 1e-4, 20.0, log=True
                ),
                'min_child_weight': trial.suggest_int(
                    'xgb_min_child_weight', 3, 12
                ),
                'gamma': trial.suggest_float(
                    'xgb_gamma', 1e-5, 1.0, log=True
                ),
                # Advanced
                'grow_policy': trial.suggest_categorical(
                    'xgb_grow_policy', ['depthwise', 'lossguide']
    ...
    if enable_lgb:
        params_lgb = {
            ...
            'colsample_bytree': trial.suggest_float(
                'lgb_colsample_bytree', 0.5, 0.9
            ),
            'reg_alpha': trial.suggest_float(
                'lgb_reg_alpha', 1e-6, 3.0, log=True
            ),
            'reg_lambda': trial.suggest_float(
                'lgb_reg_lambda', 1e-4, 6.0, log=True
            ),
            # Advanced
            'min_data_in_leaf': trial.suggest_int(
                'lgb_min_data_in_leaf', 10, 100
            ),
            'feature_fraction_bynode': trial.suggest_float(
                'lgb_feature_fraction_bynode', 0.5, 1.0
            ),
            'bagging_freq': trial.suggest_int('lgb_bagging_freq', 0, 10),
            'min_gain_to_split': trial.suggest_float(
                'lgb_min_gain_to_split', 0.0, 1.0
            ),
        }
    ...
    if enable_cat:
        params_cat = {
            ...
            'l2_leaf_reg': trial.suggest_float(
                'cat_l2_leaf_reg', 1.0, 10.0
            ),
            'subsample': trial.suggest_float('cat_subsample', 0.5, 0.95),
            'rsm': trial.suggest_float('cat_rsm', 0.5, 0.95),
            # Advanced
            'border_count': trial.suggest_int(
                'cat_border_count', 16, 255
            ),
            'random_strength': trial.suggest_float(
                'cat_random_strength', 0.1, 5.0, log=True
            ),
            'leaf_estimation_iterations': trial.suggest_int(
                'cat_leaf_estimation_iterations', 1, 20
            ),
    ...
    # ⚡ CRITICAL FIX: Set ML_HORIZONS BEFORE creating EnhancedMLSystem
    # instance
    ...
    print(
        f"[hpo] Trial {trial.number}: Processing {len(symbols)} symbols: "
        f"{symbols}",
        file=sys.stderr,
        flush=True
    )
    ...
    for sym in symbols:
        ...
        # ✅ FIX: Initialize filter variables at symbol level to avoid
        # "possibly unbound" linter errors
        # These are used in warning messages at symbol level (after split
        # loop)
        _min_mc = 0
        _min_mp = 0.0
        try:
            ...
            print(
                f"[hpo] Failed to parse HPO_MIN_MASK_COUNT, using 0: {e}",
                file=sys.stderr
            )
            _min_mc = 0
        try:
            ...
            print(
                f"[hpo] Failed to parse HPO_MIN_MASK_PCT, using 0.0: {e}",
                file=sys.stderr
            )
            _min_mp = 0.0
        print(
            f"[hpo] Trial {trial.number}: Fetching prices for {sym}...",
            file=sys.stderr,
            flush=True
        )
        ...
        if df is None:
            print(
                f"[hpo] Trial {trial.number}: {sym} - df is None, skipping",
                file=sys.stderr,
                flush=True
            )
            continue
        # ⚡ FIX: Minimum data requirement - all horizons require 100 days
        # ✅ USER REQUEST: All horizons require minimum 100 days (not
        # horizon * 15)
        # This allows more symbols to be processed, especially for longer
        # horizons
        min_required_days = 100  # All horizons require minimum 100 days
        if len(df) < min_required_days:
            print(
                f"[hpo] Trial {trial.number}: {sym} - len(df)={len(df)} < "
                f"{min_required_days} (min required for {horizon}d), "
                f"skipping",
                file=sys.stderr,
                flush=True
            )
            continue
        print(
            f"[hpo] Trial {trial.number}: {sym} - df shape: {df.shape}, "
        ...
        if not wfv_splits:
            ...
            if split_idx < 1:
                print(
                    f"[hpo] Trial {trial.number}: {sym} - Invalid split, "
                    f"skipping",
                    file=sys.stderr,
                    flush=True
                )
        ...
        print(
            f"[hpo] Trial {trial.number}: {sym} - Generated "
            f"{len(wfv_splits)} walk-forward splits",
            file=sys.stderr,
            flush=True
        )
        ...
        for split_idx, (train_end_idx, test_end_idx) in enumerate(
            ...
            print(
                f"[hpo] Trial {trial.number}: {sym} - Split "
                f"{split_idx}/{len(wfv_splits)}: train={len(train_df)} "
                f"days, test={len(test_df)} days",
                file=sys.stderr,
                flush=True
            )
            ...
            try:
                ...
                if not result:
                    print(
                        f"[hpo] Training failed for {sym} {horizon}d split "
                        f"{split_idx}",
                        file=sys.stderr,
                        flush=True
                    )
                ...
                if split_idx == 1:  # Log only for first split to avoid spam
                    print(
                        f"[hpo] {sym} {horizon}d: Training completed, model "
                        f"in memory: {models_exist}",
                        file=sys.stderr,
                        flush=True
                    )
            except Exception as e:
                print(
                    f"[hpo] Training error for {sym} {horizon}d split "
                    f"{split_idx}: {e}",
                    file=sys.stderr,
                    flush=True
                )
                ...
                if split_idx == 1:  # Log traceback only for first split
                    print(
                        f"[hpo] Traceback: {traceback.format_exc()}",
                        file=sys.stderr,
                        flush=True
                    )
                continue

            split_model_metrics = _extract_model_metrics_for_horizon(
                result, horizon
            )
            ...
            # 🔍 DEBUG: Log data source and statistics (only for first
            # split)
            if split_idx == 1:
                print(
                    f"[hpo-debug] {sym} {horizon}d: DATA SOURCE = DB "
                    f"(cache bypassed)",
                    file=sys.stderr,
                    flush=True
                )
                print(
                    f"[hpo-debug] {sym} {horizon}d: Total data: "
                    f"{total_days} days",
                    file=sys.stderr,
                    flush=True
                )
                print(
                    f"[hpo-debug] {sym} {horizon}d: Seed = "
                    f"{42 + trial.number}",
                    file=sys.stderr,
                    flush=True
                )
                # 🔍 DEBUG: Log feature flags
                feature_flags_str = ", ".join(
                    [f"{k}={v}" for k, v in feature_flags.items()]
                )
                print(
                    f"[hpo-debug] {sym} {horizon}d: Feature flags: "
                    f"{feature_flags_str}",
                    file=sys.stderr,
                    flush=True
                )
            ...
            print(
                f"[hpo-debug] {sym} {horizon}d Split {split_idx}: Train "
                f"period: {train_df.index.min()} to "
                f"{train_df.index.max()}",
                file=sys.stderr,
                flush=True
            )
            print(
                f"[hpo-debug] {sym} {horizon}d Split {split_idx}: Test "
                f"period: {test_df.index.min()} to "
                f"{test_df.index.max()}",
                file=sys.stderr,
                flush=True
            )
            ...
            valid_predictions = 0
            # Store first 5 and last 5 predictions for analysis (only first
            # split)
            prediction_details = []
            ...
            for t in range(len(test_df) - horizon):
                try:
                    cur = pd.concat(
                        [train_df, test_df.iloc[: t + 1]], axis=0
                    ).copy()
                    p = ml.predict_enhanced(sym, cur)

                    if not isinstance(p, dict):
                        # Log only first failure of first split
                        if t == 0 and split_idx == 1:
                            print(
                                f"[hpo-debug] {sym} {horizon}d t={t}: "
                                f"predict_enhanced returned "
                                f"{type(p).__name__}, expected dict",
                                file=sys.stderr,
                                flush=True
                            )
                        continue
                    ...
                    if not isinstance(obj, dict):
                        ...
                        if t == 0 and split_idx == 1:
                            print(
                                f"[hpo-debug] {sym} {horizon}d t={t}: "
                                f"prediction dict missing key '{key}' or "
                                f"value is not dict. Available keys: "
                                f"{list(p.keys())}",
                                file=sys.stderr,
                                flush=True
                            )
                    ...
                    if (
                        ...
                        if last_close > 0:
                            ...
                            # Store prediction details for first 5 and last
                            # 5 (only first split)
                            if split_idx == 1 and (
                                t < 5 or t >= len(test_df) - horizon - 5
                            ):
                                true_return = (
                                    y_true.iloc[t]
                                    if not np.isnan(y_true.iloc[t])
                                    else np.nan
                                )
                                pred_details = {
                                    't': t,
                    ...
                    # Log only first failure of first split
                    elif t == 0 and split_idx == 1:
                        print(
                            f"[hpo-debug] {sym} {horizon}d t={t}: "
                            f"ensemble_prediction invalid: {pred_price} "
                            f"(type: {type(pred_price).__name__})",
                            file=sys.stderr,
                            flush=True
                        )
                except Exception as e:
                    # Log only first exception of first split
                    if t == 0 and split_idx == 1:
                        print(
                            f"[hpo-debug] {sym} {horizon}d t={t}: Prediction "
                            f"exception: {e}",
                            file=sys.stderr,
                            flush=True
                        )
            ...
            if split_idx == 1:
                print(
                    f"[hpo-debug] {sym} {horizon}d: Valid predictions: "
                    f"{valid_predictions}/{len(test_df) - horizon}",
                    file=sys.stderr,
                    flush=True
                )
                if prediction_details:
                    print(
                        f"[hpo-debug] {sym} {horizon}d: Sample predictions "
                        f"(first/last 5):",
                        file=sys.stderr,
                        flush=True
                    )
                    for pred_detail in prediction_details[:5]:
                        print(
                            f"[hpo-debug]   t={pred_detail['t']}, "
                            f"date={pred_detail['date']}, "
                            f"pred_return={pred_detail['pred_return']:.4f}, "
                            f"true_return="
                            f"{pred_detail['true_return']:.4f}, "
            ...
            # ✅ FIX: Initialize low_support variables before conditional
            # block (linter fix)
            low_support = False
            # Note: _min_mc and _min_mp are initialized at symbol level
            # (above) to avoid linter errors

            # 🔍 DEBUG: Calculate additional metrics
            if valid_count > 0:
                # DirHit
                dirhit_val = dirhit(
                    y_true.values[valid_mask], preds[valid_mask]
                )
                ...
                if split_idx == 1:
                    print(
                        f"[hpo-debug] {sym} {horizon}d: METRICS:",
                        file=sys.stderr,
                        flush=True
                    )
                    print(
                        f"[hpo-debug]   Valid predictions: "
                        f"{valid_count}/{len(preds)}",
                        file=sys.stderr,
                        flush=True
                    )
                    print(
                        f"[hpo-debug]   DirHit (threshold={thr}): "
                        f"{dirhit_val:.2f}% (mask_count={mask_count}, "
                        f"mask_pct={mask_pct:.1f}%)",
                        file=sys.stderr,
                        flush=True
                    )
                    print(
                        f"[hpo-debug]   Direction match (all): "
                        f"{direction_pct:.2f}% "
                    ...
                    print(
                        f"[hpo-debug]   RMSE: {rmse:.6f}",
                        file=sys.stderr,
                        flush=True
                    )
                    print(
                        f"[hpo-debug]   MAPE: {mape:.2f}%",
                        file=sys.stderr,
                        flush=True
                    )
                ...
                try:
                    ...
                    logger.debug(
                        f"Failed to calculate NRMSE for split: {e}"
                    )
                # ✅ NEW: Low-support guard to prevent spurious 100% DirHit
                # on tiny masks
                # Configure via environment:
                #  - HPO_MIN_MASK_COUNT (int, default 0 → disabled)
                #  - HPO_MIN_MASK_PCT (float percent, default 0 → disabled)
                # Note: low_support, _min_mc, _min_mp already initialized
                # above (moved to top of block)
                if (
                    ...
                    if split_idx == 1:
                        print(
                            f"[hpo] {sym} {horizon}d Split {split_idx}: "
                            f"LOW_SUPPORT → exclude from avg "
                            f"(DirHit={dirhit_val:.2f}%, "
                    ...
                    print(
                        ...
                        f"mask={mask_count}, RMSE={rmse:.6f}, "
                        f"MAPE={mape:.2f}%)",
                        file=sys.stderr,
                        flush=True
                    )
                ...
                print(
                    f"[hpo] {sym} {horizon}d Split {split_idx}: No valid "
                    f"predictions! "
            ...
            split_entry: Dict[str, Any] = {
                ...
                'test_end': (
                    test_df.index.max().isoformat()
                    if len(test_df) > 0 else None
                ),
                'valid_predictions': int(valid_count),
                'model_metrics': split_model_metrics,
                # If low support, mark dirhit as None to signal exclusion
                # from averages
                'dirhit': (
                    float(dirhit_val)
                    if (dirhit_val is not None and not low_support)
                    else None
                ),
                'rmse': (
                    float(rmse)
                    if isinstance(rmse, (int, float))
                    and math.isfinite(rmse) else None
                ),
                'mape': (
                    float(mape)
                    if isinstance(mape, (int, float))
                    and math.isfinite(mape) else None
                ),
                'nrmse': (
                    float(nrmse_val)
                    if isinstance(nrmse_val, (int, float))
                    and math.isfinite(nrmse_val) else None
                ),
        ...
        if len(split_dirhits) >= 2:
            ...
            print(
                f"[hpo] {sym} {horizon}d: Average DirHit across "
                f"{len(split_dirhits)} splits: {avg_dirhit_value:.2f}% "
            ...
            print(
                f"[hpo] {sym} {horizon}d: Only 1 valid split "
                f"(DirHit={split_dirhits[0]:.2f}%) - excluded (need at "
                f"least 2 splits for reliability)",
                file=sys.stderr,
                flush=True
            )
            # ✅ WARNING: Only 1 split - statistically unreliable
            print(
                f"⚠️ WARNING: {sym} {horizon}d: Only 1 split passed filter "
                f"(min_count={_min_mc}, min_pct={_min_mp}%) - "
                f"DirHit excluded (need at least 2 splits for statistical "
                f"reliability)",
                file=sys.stderr,
                flush=True
            )
            ...
            print(
                f"[hpo] {sym} {horizon}d: No valid DirHit from any split",
                file=sys.stderr,
                flush=True
            )
            ...
            print(
                f"⚠️ WARNING: {sym} {horizon}d: All splits excluded by "
                f"filter (min_count={_min_mc}, min_pct={_min_mp}%) - "
                f"best params may not be optimal for this symbol",
                file=sys.stderr,
                flush=True
            )
            symbol_metric_entry['low_support_warning'] = True
        # Compute per-symbol nRMSE as the average across split nRMSE
        # values
        ...
        if split_nrmses_local:
            try:
                ...
                logger.debug(
                    f"Failed to calculate avg_nrmse_local: {e}"
                )
        symbol_metric_entry['avg_dirhit'] = avg_dirhit_value
        symbol_metric_entry['avg_nrmse'] = avg_nrmse_value
        symbol_metric_entry['split_count'] = len(
            symbol_metric_entry['split_metrics']
        )
        symbol_metric_entry['avg_model_metrics'] = (
    ...
    if not dirhits:
        print(
            f"[hpo] Trial {trial.number}: No DirHit values calculated for "
            f"any symbol! Returning 0.0",
            file=sys.stderr,
            flush=True
        )
    ...
    score = float(
        0.7 * avg_dirhit
        - k * (avg_nrmse if np.isfinite(avg_nrmse) else 3.0)
    )
    print(
        f"[hpo] Trial {trial.number}: Average DirHit={avg_dirhit:.2f}% "
        f"(from {len(dirhits)} symbols), nRMSE={avg_nrmse:.3f}, "
        f"score={score:.2f}",
        file=sys.stderr,
        flush=True
    )
    try:
        ...
        # ✅ FIX: Store symbol_metrics in trial user_attrs so it can be
        # retrieved later for best_trial_metrics
        if trial_symbol_metrics:
            ...
            print(
                f"[hpo-debug] Trial {trial.number}: Attempting to save "
                f"symbol_metrics: {len(metrics_keys)} symbols, "
                f"size={metrics_size:,} bytes",
                file=sys.stderr,
                flush=True
            )
            try:
                ...
                print(
                    f"[hpo-debug] Trial {trial.number}: ✅ symbol_metrics "
                ...
                print(
                    f"[hpo-error] Trial {trial.number}: ❌ Failed to save "
                    f"symbol_metrics: "
                    f"{type(sym_metrics_err).__name__}: {sym_metrics_err}",
                    file=sys.stderr,
                    flush=True
                )
                import traceback
                print(
                    f"[hpo-error] Trial {trial.number}: Traceback:",
                    file=sys.stderr,
                    flush=True
                )
                print(
        ...
        print(
            f"[hpo-error] Trial {trial.number}: ❌ Failed to save "
            f"user_attrs: {type(e).__name__}: {e}",
            file=sys.stderr,
            flush=True
        )
        import traceback
    ...
    # ⚡ MEMORY LEAK FIX: Clean up models after each trial to prevent
    # memory leak
    # Models accumulate in ml.models dictionary during training, causing RAM
    ...
    if hasattr(ml, 'horizon_features'):
        ml.horizon_features.clear()  # type: ignore[attr-defined]
    if hasattr(ml, '_feature_cache'):
        ml._feature_cache.clear()  # type: ignore[attr-defined]
...
def main():
    ...
    print()
    print(
        "⚡ NEW: TÜM Feature flags + TÜM Feature iç parametreleri + "
        "Hyperparameters birlikte optimize ediliyor!"
    )
    print("   - Feature flags: açık/kapalı (11 adet)")
    print(
        "     * ENABLE_EXTERNAL_FEATURES, ENABLE_FINGPT_FEATURES, "
        "ENABLE_YOLO_FEATURES"
    )
    print(
        "     * ML_USE_DIRECTIONAL_LOSS, ENABLE_SEED_BAGGING, "
        "ENABLE_TALIB_PATTERNS"
    )
    print(
        "     * ML_USE_SMART_ENSEMBLE, ML_USE_STACKED_SHORT, "
        "ENABLE_META_STACKING"
    )
    print("     * ML_USE_REGIME_DETECTION, ENABLE_FINGPT")
    print(
        "   - Feature iç parametreleri (10-12 adet, feature açık/kapalı "
        "durumuna göre):"
    )
    print(
        "     * Directional Loss (3): ml_loss_mse_weight, "
        "ml_loss_threshold, ml_dir_penalty"
    )
    print("     * Seed Bagging (1): n_seeds")
    print("     * Meta Stacking (1): meta_stacking_alpha")
    print(
        "     * Adaptive Learning (2): ml_adaptive_k_{horizon}d, "
        "ml_pattern_weight_scale_{horizon}d"
    )
    print("     * YOLO (1): yolo_min_conf")
    print(
        "     * Smart Ensemble (3): smart_consensus_weight, "
        "smart_performance_weight, smart_sigma"
    )
    print("     * FinGPT (1): fingpt_confidence_threshold")
    print(
        "   - Hyperparameters: model parametreleri (~15-20 adet: XGBoost, "
        "LightGBM, CatBoost)"
    )
    print(
        "   - Not: ML_USE_ADAPTIVE_LEARNING her zaman kapalı (HPO'da data "
        "leakage önleme)"
    )
    print(
        "   - Toplam optimize edilen parametre: ~36-43 adet (11 flag + "
        "10-12 feature param + 15-20 hyperparam)"
    )
    print()

    # Use DATABASE_URL from environment (PgBouncer-friendly); fallback to
    # direct Postgres
    db_url = os.getenv(
        'DATABASE_URL',
    ...
    def _estimate_days(sym: str) -> int:
        try:
            ...
            logger.debug(
                f"Failed to estimate days for {sym}: {e}"
            )
            return 0
    ...
    # ✅ CRITICAL FIX: Read target_trials from environment variable
    # (default: 1500)
    try:
        target_trials = int(os.getenv('HPO_TRIALS', '1500'))
    except Exception as e:
        logger.debug(
            f"Failed to get HPO_TRIALS, using 1500: {e}"
        )
    ...
    print(
        f"Trials (effective): {n_trials}  [min_days={min_days}]"
    )
    print(
        f"Timeout (effective): {timeout}s ({timeout/3600:.1f}h)"
    )

    # Create study
    # ✅ CRITICAL FIX: Add cycle number to study name to allow new HPO in
    # each cycle
    # Each cycle should have its own study file to enable incremental
    # learning with new data
    # Previous cycles' studies are preserved for analysis
    symbol_str = (
        '_'.join(symbols)
        if len(symbols) <= 2
        else f"{symbols[0]}_and_{len(symbols)-1}more"
    )
    # Get cycle number from environment (set by pipeline) or default to 1
    cycle_num = int(os.getenv('HPO_CYCLE', '1'))
    study_name = (
        f"hpo_with_features_{symbol_str}_h{horizon}_c{cycle_num}"
    )
    study_path = (
        f"sqlite:////opt/bist-pattern/hpo_studies/{study_name}.db"
    )
    ...
    # WAL allows multiple readers and one writer simultaneously, reducing
    # lock contention
    # This is critical when 96 HPO processes are writing to SQLite files
    # concurrently
    try:
        import sqlite3
        ...
        if os.path.exists(db_path):
            ...
            conn.close()
            print(
                f"✅ Enabled WAL mode for {study_name}",
                flush=True
            )
    except Exception as e:
        print(
            f"⚠️ Could not enable WAL mode: {e}",
            flush=True
        )
    ...
    # ✅ Warm-start: enqueue best params from recent JSON results
    # (top-K = 3)
    try:
        # Skip warm-start if a specific model is forced (to avoid fixed
        # param mismatch)
        if os.getenv('FORCE_MODEL_CHOICE'):
            print(
                "⏭️ Skipping warm-start enqueue (FORCE_MODEL_CHOICE is set)",
                flush=True
            )
        else:
            import glob
            ...
            for jf in prev_jsons:
                ...
                if any(s in prev_syms for s in symbols):
                    ...
                    if isinstance(bp, dict) and bp:
                        ...
                        enqueued += 1
                        print(
                            f"↪️ Warm-start: enqueued params from "
                            f"{os.path.basename(jf)}",
                            flush=True
                        )
    except Exception as e:
        logger.debug(f"Warm-start enqueue failed: {e}")

    # ✅ FIX: Check existing trials before optimizing to prevent exceeding
    # n_trials limit
    # When multiple processes write to the same study, each process may try
    # to add n_trials,
    ...
    if remaining_trials <= 0:
        print(
            f"✅ Study already has {existing_trials} trials "
            f"(target: {n_trials}), skipping optimization",
            flush=True
        )
    else:
        print(
            f"📊 Study has {existing_trials} existing trials, will add "
            f"{remaining_trials} more (target: {n_trials})",
            flush=True
        )
    # Optimize
    # ✅ FIX: Pass study and n_trials to objective for per-trial limit
    # checking
    study.optimize(
        lambda trial: objective(
            trial, symbols, horizon, engine, db_url, study=study,
            max_trials=n_trials
        ),
    ...
    # Save results to JSON (compatible with
    # continuous_hpo_training_pipeline.py)
    best_value = (
        float(study.best_value)
        if study.best_value is not None else 0.0
    )
    ...
    # Feature parameters: ml_loss_*, n_seeds, meta_stacking_alpha,
    # ml_adaptive_k_*, ml_pattern_weight_scale_*,
    # yolo_min_conf, smart_consensus_weight, smart_performance_weight,
    # smart_sigma, smart_weight_*, fingpt_confidence_threshold
    feature_params_keys = [
    ...
    feature_params_keys += [
        ...
        if (
            ...
            or k.startswith('ml_pattern_weight_scale_')
        )
    ]
    feature_params = {
        k: v for k, v in best_params.items()
    ...
    output_file = (
        f"/opt/bist-pattern/results/"
        f"optuna_pilot_features_on_h{horizon}_c{cycle_num}_"
    ...
    # Best dirhit from trial user attrs if available (objective returns
    # combined score)
    best_dirhit = None
    try:
    ...
    try:
        sorted_trials = sorted(
            ...
            key=lambda tr: (
                float(tr.value)
                if tr.value is not None else float('-inf')
            ),
            reverse=True
        )
        topk = []
    ...
    result = {
        ...
        # DirHit percentage (from user_attrs or fallback)
        ...
        'best_trial': {
            ...
            'value': (
                ...
                if best_trial.value is not None else 0.0
            ),
        ...
        # NEW: Feature flags that were optimized
        'feature_flags': feature_flags,
        # NEW: Feature internal parameters
        'feature_params': feature_params,
        # NEW: Separated hyperparameters
        'hyperparameters': hyperparameters,
        'features_enabled': {
            'ENABLE_EXTERNAL_FEATURES': (
                '1' if feature_flags.get(
            ...
            'ENABLE_FINGPT': (
                '1' if feature_flags.get('enable_fingpt', False)
                else '0'
            ),
            # Always disabled during HPO (data leakage önleme)
            'ML_USE_ADAPTIVE_LEARNING': '0',
            # Algorithm enables per best model_choice
            'ENABLE_XGBOOST': (
                '1' if best_params.get('model_choice') in ('xgb', 'all')
                else '0'
            ),
            'ENABLE_LIGHTGBM': (
                '1' if best_params.get('model_choice') in ('lgbm', 'all')
                else '0'
            ),
            'ENABLE_CATBOOST': (
                '1' if best_params.get('model_choice') in ('cat', 'all')
                else '0'
            ),
        }
    }
    ...
    if isinstance(symbol_metrics_best, dict):
        ...
        for sym_key, sym_metrics in symbol_metrics_best.items():
            if (
                isinstance(sym_metrics, dict)
                and sym_metrics.get('low_support_warning')
            ):
                # Extract symbol and horizon from key (e.g., "ADEL_1d" ->
                # "ADEL", 1)
                ...
                if len(parts) == 2:
                    ...
                    try:
                        ...
                        logger.debug(
                            f"Failed to parse horizon from {parts[1]}: {e}"
                        )
        if low_support_symbols:
            result['low_support_warnings'] = low_support_symbols
            print(
                f"⚠️ WARNING: {len(low_support_symbols)} symbol(s) had "
                f"all splits excluded by filter: "
                f"{', '.join(low_support_symbols)}",
                file=sys.stderr,
                flush=True
            )
    ...
    try:
        ...
        try:
            ...
            _thr = 0.005
        # ✅ CRITICAL FIX: Get filter values from best_trial's
        # split_metrics (what was actually used during HPO)
        # Not from environment variables (which may have changed)
        _min_mc_spec = 0
        _min_mp_spec = 0.0
        try:
            if (
                isinstance(symbol_metrics_best, dict)
                and len(symbol_metrics_best) > 0
            ):
                # Get filter values from first symbol's first split (all
                # should have same filter)
                first_symbol_metrics = next(
                    iter(symbol_metrics_best.values())
                )
                split_metrics = first_symbol_metrics.get(
                    'split_metrics', []
                )
                if split_metrics:
                    first_split = split_metrics[0]
                    _min_mc_spec = int(
                        first_split.get('min_mask_count', 0)
                    )
                    _min_mp_spec = float(
                        first_split.get('min_mask_pct', 0.0)
                    )
        except Exception as e:
            logger.debug(
                f"Failed to get filter values from split_metrics: {e}"
            )
            # Fallback to environment variables if split_metrics not
            # available
        ...
        try:
            ...
            logger.debug(
                f"Failed to set scoring config, using 6.0: {e}"
            )
            _k = 6.0
        ...
        if isinstance(symbol_metrics_best, dict):
            for sym_key, metrics in symbol_metrics_best.items():
                ...
                for s in metrics.get('split_metrics', []):
                    splits_out.append({
                        'split_index': (
                            int(s.get('split_index'))
                            if 'split_index' in s else None
                        ),
                        'train_end_idx': (
                            int(s.get('train_end_idx'))
                            if s.get('train_end_idx') is not None else None
                        ),
                        'test_end_idx': (
                            int(s.get('test_end_idx'))
                            if s.get('test_end_idx') is not None else None
                        ),
    ...
    # Write to temp file first, then atomic rename to prevent partial
    # writes
    json_saved = False
    try:
        ...
        try:
            ...
            with open(tmp_file, 'w') as f:
                ...
                try:
                    ...
                    logger.debug(
                        f"fsync failed (may not be available): {e}"
                    )
        ...
        print(
            f"❌ ERROR: Failed to save JSON file {output_file}: {json_err}",
            file=sys.stderr,
            flush=True
        )
        import traceback
        ...
        # ✅ CRITICAL FIX: Try to recover by reading best params from
        # study and recreating JSON
        # This ensures we don't lose 1500+ trials of work
        print(
            "🔄 Attempting to recover JSON file from study...",
            flush=True
        )
        try:
            ...
            output_file_recovered = output_file.replace(
                '.json', '_recovered.json'
            )
            tmp_file_recovered = output_file_recovered + '.tmp'
            try:
                ...
                print(
                    f"✅ Recovered JSON file saved to: "
                    f"{output_file_recovered}",
                    flush=True
                )
                ...
                try:
                    ...
                    logger.debug(
                        f"Failed to remove tmp_file_recovered: {e}"
                    )
                raise recover_write_err
        except Exception as recover_err:
            print(
                f"❌ ERROR: Failed to recover JSON file: {recover_err}",
                file=sys.stderr,
                flush=True
            )
            print(
                f"⚠️ WARNING: JSON file not saved, but study file exists "
                f"at {study_path}",
                file=sys.stderr,
                flush=True
            )
            print(
                "⚠️ WARNING: Best params can be recovered from study file "
                "later",
                file=sys.stderr,
                flush=True
            )
    ...
    if output_file and json_saved:
        try:
            from bist_pattern.utils.file_utils import (  # type: ignore
            ...
            print(
                f"⚠️ WARNING: Failed to set file permissions for "
                f"{output_file}: {perm_err}",
                file=sys.stderr,
                flush=True
            )
    ...
    if output_file and json_saved:
        ...
        print(
            f"⚠️ WARNING: Results NOT saved to JSON file (study file "
            f"exists at {study_path})"
        )
    ...
    if not json_saved:
        print(
            "❌ FATAL: JSON file not saved after recovery attempts",
            file=sys.stderr,
            flush=True
        )
