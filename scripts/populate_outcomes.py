...
Policy:
  - For horizon 'Xd', evaluate at ts_pred + X days (market day
    granülarite: fiyat olarak DB'deki son tarih<=eval_ts kullanılır).
...
Timezone: Converts UTC timestamps to Istanbul market time for accurate
date comparisons.
"""
...
def run(limit: int = 1000) -> int:
    with app.app_context():
        ...
        # For each (symbol, horizon, DATE(ts_pred)), select the LATEST
        # prediction made AFTER market close
        # Market close: ~18:00 Istanbul time (15:00 UTC + 3 hours = 18:00
        # local)
        # This ensures we use end-of-day predictions with complete market
        # data, not overnight training predictions

        # Subquery: Find latest ts_pred per (symbol, horizon, date) made
        ...
        subq = (
            db.session.query(
                func.upper(func.trim(PredictionsLog.symbol)).label(
                    'clean_symbol'
                ),
            ...
            # After market close (local time)
            .filter(local_hour_expr >= eod_cutoff_hour_local)
            # ✅ FIX: Process ALL horizons (7d, 14d, 30d were missing!)
            .filter(
                PredictionsLog.horizon.in_(['1d', '3d', '7d', '14d', '30d'])
            )
            .group_by(
                func.upper(func.trim(PredictionsLog.symbol)),
                PredictionsLog.horizon,
                pred_date
            )
            .subquery()
        ...
        # IMPORTANT: Also filter main query for EOD predictions to ensure
        # we only get 18:00+ predictions
        # Recompute local_hour_expr for main query (SQLAlchemy needs fresh
        # expression)
        if tz_off != 0:
            main_local_hour = func.extract(
                'hour',
                PredictionsLog.ts_pred + text(f"INTERVAL '{tz_off} hours'")
            )
        else:
        ...
        q = (
            ...
            # Ensure main query also filters for EOD
            .filter(main_local_hour >= eod_cutoff_hour_local)
            .order_by(
                subq.c.pred_date.asc(),
                func.upper(func.trim(PredictionsLog.symbol)).asc()
            )
        )
        ...
        if candidates:
            ...
            logger.info(
                f"📊 Found {len(candidates)} daily snapshot predictions "
                f"(EOD>=18:00) | "
                f"Sample: {sample_pred.symbol} @ "
                f"{local_ts.strftime('%Y-%m-%d %H:%M')} local"
            )
        else:
            logger.info(
                f"📊 Found {len(candidates)} daily snapshot predictions "
                f"without outcomes (latest per symbol/horizon/day)"
            )
        ...
        for p in candidates:
            ...
            price_now = (
                float(p.price_now) if p.price_now is not None else None
            )
            pred_price = (
                float(p.pred_price) if p.pred_price is not None else None
            )
            ...
            if price_now is None or pred_price is None:
                ...
                continue
            delta_real = (
                (price_eval - price_now) / price_now
                if price_now else 0.0
            )
            delta_pred = (
                float(p.delta_pred)
                if p.delta_pred is not None
                else (pred_price - price_now) / price_now
            )

            # Direction hit with threshold-based logic (fixes asymmetric zero
            # bias)
            # Threshold: 0.5% - aligned with typical model prediction scale
            # Adjusted from 1.0% to 0.5% to match conservative model
            # predictions
            threshold = float(os.getenv('DIRECTION_HIT_THRESHOLD', '0.005'))
            real_dir = (
                0 if abs(delta_real) < threshold
                else (1 if delta_real > 0 else -1)
            )
            pred_dir = (
                0 if abs(delta_pred) < threshold
                else (1 if delta_pred > 0 else -1)
            )

            # Logic:
            # - Both zero (no significant change) → HIT
            # - One zero, one non-zero → MISS (prediction failed to capture
            #   movement or predicted false movement)
            # - Same direction → HIT
            # - Opposite direction → MISS
            if real_dir == 0 and pred_dir == 0:
                # Both predicted and realized no significant change
                dir_hit = True
            elif real_dir == 0 or pred_dir == 0:
                # One changed significantly, other didn't
                dir_hit = False
            else:
            ...
            # ⚡ NEW: Magnitude-based hit calculation (keeps dir_hit unchanged
            # for backward compatibility)
            # Calculate additional metrics for better evaluation:
            # 1. magnitude_hit: Direction is correct AND magnitude is within
            #    tolerance
            # 2. partial_hit: Partial score based on both direction and
            #    magnitude accuracy
            # Note: These variables are calculated but not stored in DB (can
            # be computed on-the-fly when needed)
            _magnitude_hit = None  # noqa: F841
            _partial_hit = None  # noqa: F841
            try:
                # Magnitude tolerance: consider it a "magnitude hit" if
                # abs_err is within 5% of actual price or if both predicted
                # and actual movements are small (within threshold)
                # 5% default
                magnitude_tolerance = float(
                    os.getenv('MAGNITUDE_HIT_TOLERANCE', '0.05')
                )

                # Direction hit (already calculated)
                dir_correct = dir_hit

                # Magnitude hit: direction correct AND price error within
                # tolerance
                if dir_correct:
                    # If both movements are small (within threshold),
                    # consider it a magnitude hit
                    if (
                        abs(delta_real) < threshold
                        and abs(delta_pred) < threshold
                    ):
                        # Both predicted and realized small/no movement
                        _magnitude_hit = True  # noqa: F841
                    else:
                        # Check if price error is within tolerance
                        if (
                            price_eval
                            and abs_err / price_eval <= magnitude_tolerance
                        ):
                            _magnitude_hit = True  # noqa: F841
                        else:
                            _magnitude_hit = False  # noqa: F841
                else:
                    # Direction wrong, cannot be magnitude hit
                    _magnitude_hit = False  # noqa: F841

                # Partial hit: weighted score combining direction and magnitude
                # Score: 0.0 (completely wrong) to 1.0 (perfect prediction)
                # Note: These variables are calculated but not stored in DB
                ...
                if dir_correct:
                    ...
                    if price_eval and price_eval > 0:
                        ...
                        normalized_error = min(
                            1.0,
                            abs_err / (price_eval * magnitude_tolerance)
                        )
                    ...
                    # Direction wrong: partial hit based on how close the
                    # magnitude was
                    if price_eval and price_eval > 0:
                        normalized_error = min(
                            1.0,
                        ...
                        # Even if direction is wrong, give some credit for
                        # magnitude accuracy (max 0.3)
                        _partial_hit = (
                            0.3 * (1.0 - normalized_error)
                        )  # noqa: F841
                    else:
            ...
            # Note: magnitude_hit and partial_hit are calculated but not
            # stored in DB
            # They can be computed on-the-fly when needed (backward
            # compatible)
            db.session.add(o)
            processed += 1
        try:
        ...
        print(
            ...
            f"skips={{no_days:{skipped_no_days},"
            f"not_matured:{skipped_not_matured},"
            f"no_stock:{skipped_no_stock},"
            f"no_price:{skipped_no_price},"
            f"missing_prices:{skipped_missing_prices}}}"
        )
    return 0
