...
import os, pathlib, random, sys, collections
...
with app.app_context():
    ...
    WINDOW=int(os.getenv("WINDOW","120")); STRIDE=int(os.getenv("STRIDE","4"))
    PER=int(os.getenv("PER_SYMBOL_MAX","200")); TR=float(os.getenv("TRAIN_RATIO","0.98"))
    FORCE_TRAIN_NEW=os.getenv("FORCE_TRAIN_NEW","1") in ("1","true","True")
    CLASS_CAP=int(os.getenv("CLASS_CAP","50000"))
    ...
    NEW_CLASSES={"wedge_up", "wedge_down", "flag", "pennant", "cup_handle", "channel_up", "channel_down"}
    ...
    def render(seg,out):
        try:
            ...
            ax.plot(range(len(seg)),seg,color='#1f77b4',linewidth=2); ax.set_axis_off(); f.tight_layout(pad=0)
    ...
    for sym in symbols:
        try:
            ...
            for st in range(0,max(1,n-WINDOW-1),STRIDE):
                ...
                if not lab: continue
                typ,a,b=lab
                if totals[typ]>=CLASS_CAP: continue
                cid=cls_map[typ]
                fname=f"{sym}_more_{typ}_{st}.png"
                is_tr = True if (FORCE_TRAIN_NEW and typ in NEW_CLASSES) else (random.random()<TR)
                ...
                if render(seg,img):
                    ...
                    totals[typ]+=1; written+=1
        except Exception:
            continue
    print("more_patterns_written_total:", sum(totals.values()))
    for k,v in totals.items(): print(k, v)
