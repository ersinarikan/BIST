#!/usr/bin/env python3
"""
Find fallback best params for symbols with low support (all splits excluded by filter)

This script finds best params using 0/0.0 filter (no filter) when the original filter
excludes all splits. This ensures we have valid params even for low-support symbols.
"""
...
def find_fallback_best_params(study_db: Path, symbol: str, horizon: int) -> Optional[Dict]:
    ...
    Returns:
        Dict with best_params, best_trial_number, features_enabled, feature_params
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Find fallback best params for low-support symbols')
    parser.add_argument('--study-db', type=str, required=True, help='Path to Optuna study database')
    ...
    if result:
        print(f"✅ Found fallback best params for {args.symbol} {args.horizon}d")
