...
def compute_metrics(day: date) -> None:
    with app.app_context():
        ...
        for (symbol, horizon), items in buckets.items():
            ...
            mae = (
                max(0.0, min(9999.9999, mae_raw))
                if mae_raw is not None else None
            )
            ...
            items_sorted = sorted(
                items,
                key=lambda x: getattr(x[1], 'ts_eval', None) or datetime.min
            )
            for p, o in items_sorted:
                ...
                if predicted and hit:
                    ...
                    fn += 1
                # PnL: take trades only when predicted; long if d_pred>=0
                # else short
                if predicted:
            ...
            sharpe = (
                _compute_sharpe(trade_returns) if trade_returns else None
            )
            max_dd = (
                _compute_max_drawdown(trade_returns)
            ...
            if pnl_pct is not None:
                pnl_clamped = max(
                    -999999999999.99,
                    min(999999999999.99, pnl_pct * 100.0)
                )
                rec.pnl = pnl_clamped
            else:
                rec.pnl = None
