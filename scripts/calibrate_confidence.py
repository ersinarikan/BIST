...
Note: Isotonic bins were removed and replaced by online adjustment in
pattern_detector.py
"""
...
def isotonic_bins(
    ...
    """DEPRECATED: Build reliability bins (c_in -> c_out) using sklearn
    IsotonicRegression.

    This function is no longer used. Isotonic calibration was replaced by
    online adjustment which directly queries the database for recent
    outcomes.
...
def optimize_thresholds(
    pairs: List[Tuple[float, float]], deltas: List[float]
) -> Dict[str, float]:
...
def optimize_thresholds_challenger(
    pairs: List[Tuple[float, float]],
    deltas: List[float],
    prod_th: Dict[str, float]
) -> Dict[str, float]:
    ...
    """
    if not pairs or not deltas:
        # Default challenger values (slightly more aggressive than production
        # default)
    ...
    best = {
        'delta_thr': max(0.01, prod_delta * 0.75),
        'conf_thr': max(0.50, prod_conf * 0.92)
    }
    best_f1 = -1.0
    ...
    delta_candidates = sorted(
        set([round(d, 4) for d in delta_candidates if 0.01 <= d <= 0.05])
    )
    ...
    conf_candidates = sorted(
        set([round(c, 3) for c in conf_candidates if 0.50 <= c <= 0.80])
    )
...
def analyze_ab_test_results(
    cutoff: datetime, horizon: str, min_samples_chall: int = 50
) -> Dict[str, Any]:
    ...
    # Check if challenger is significantly better (at least 2% improvement
    # with enough samples)
    chall_better = False
    significant = False
    if (
        prod_acc is not None
        and chall_acc is not None
        and chall_n >= min_samples_chall
    ):
    ...
    return {
        ...
        'improvement': (
            (chall_acc - prod_acc)
            if (prod_acc is not None and chall_acc is not None)
            else None
        )
    }
...
def run(
    ...
    min_samples: int = 150,
    enable_challenger_opt: bool = False,
    enable_production_promote: bool = False,
    min_samples_chall: int = 50,
    ...
    if go_live:
        try:
            ...
            # Avoid overly narrow window: ensure at least half window after
            # go_live
    ...
    # horizon -> info
    state: Dict[str, Dict[str, int | bool | str]] = {'meta': {}}
        ...
        for h in horizons:
            ...
            for p, o in q.all():
                try:
                    c = (
                        float(p.confidence)
                        if p.confidence is not None else None
                    )
                    if c is None:
                        continue
                    ...
                    d = (
                        float(p.delta_pred)
                        if p.delta_pred is not None else 0.0
                    )
                    deltas.append(abs(d))
                except Exception:
                    continue
            ...
            if len(pairs) < min_samples:
                store['skipped_horizons'].append({
                    'horizon': h,
                    'reason': f'not_enough_samples:{len(pairs)}'
                })
                prev = (
                    prev_store.get('horizons', {})
                    if isinstance(prev_store, dict) else {}
                ).get(h)
                used_prev = False
                if prev:
                ...
                state[h] = {
                    ...
                    'used_default': (not used_prev),
                }
                continue
            # ⚡ REMOVED: Isotonic bins no longer used (replaced by online
            # adjustment)
            # bins = isotonic_bins(pairs, bins=10)  # DEPRECATED: Not used in
            # prediction
            th = optimize_thresholds(pairs, deltas)
            # Only store thresholds (isotonic bins removed)
            ...
            # This ensures we promote the newly optimized challenger, not old
            # values
            # Will hold newly optimized challenger thresholds
            optimized_chall_th = None

            if enable_challenger_opt:
                # Analyze A/B test results for this horizon
                ab_results = analyze_ab_test_results(
                    cutoff, h, min_samples_chall=min_samples_chall
                )

                # Option 2: Optimize challenger thresholds using A/B test data
                # (DO THIS FIRST)
                ...
                if len(chall_pairs) >= min_samples_chall:
                    chall_th = optimize_thresholds_challenger(
                        chall_pairs, chall_deltas, th
                    )
                    ...
                    # Save optimized challenger thresholds
                    optimized_chall_th = chall_th
                    print(
                        f"[challenger-opt] {h}: Optimized challenger "
                        f"thresholds "
                        f"(delta_thr: {chall_th['delta_thr']}, "
                        f"conf_thr: {chall_th['conf_thr']}, "
                        f"samples: {len(chall_pairs)})",
                        flush=True
                    )
                else:
                    # Not enough challenger samples, preserve existing
                    # challenger config
                    prev_bandit = prev_store.get('bandit', {})
                    prev_bandit_h = (
                        prev_bandit.get('horizons', {}) or {}
                    ...
                    if prev_bandit_h and prev_bandit_h.get('challenger'):
                        ...
                        # Use existing if not optimized
                        optimized_chall_th = prev_bandit_h.get('challenger')
                        print(
                            f"[challenger-opt] {h}: Not enough samples "
                            f"({len(chall_pairs)} < {min_samples_chall}), "
                            f"preserving existing challenger config",
                            flush=True
                        )

                # Option 1: Promote challenger to production if significantly
                # better (DO THIS AFTER OPTIMIZATION)
                # ✅ FIX: Use newly optimized challenger thresholds, not old
                # ones from prev_store
                if (
                    enable_production_promote
                    and ab_results['significant']
                    and ab_results['chall_better']
                ):
                    # Use optimized challenger thresholds (if available) or
                    # existing ones
                    chall_to_promote = (
                        optimized_chall_th
                        or prev_store.get('bandit', {})
                        .get('horizons', {})
                        .get(h, {})
                        .get('challenger', {})
                    )
                    ...
                    if chall_to_promote:
                        ...
                        print(
                            f"[challenger-promote] {h}: Promoting challenger "
                            f"to production "
                            f"(improvement: "
                            f"{ab_results['improvement']*100:.2f}%, "
                            f"prod_acc: "
                            f"{ab_results['prod']['acc']*100:.1f}%, "
                            f"chall_acc: "
                            f"{ab_results['chall']['acc']*100:.1f}%)",
                            flush=True
                        )
                        # Use optimized challenger thresholds as production
                        th = chall_to_promote.copy()
                        store['horizons'][h]['thresholds'] = th
    ...
    try:
        ...
        _h = hashlib.sha256(
            json.dumps(store.get('horizons', {}), sort_keys=True).encode(
                'utf-8'
            )
        ).hexdigest()
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--go-live',
        type=str,
        default=os.getenv('CALIB_GO_LIVE', '')
    )
    parser.add_argument(
        '--min-samples',
        type=int,
        default=int(os.getenv('CALIB_MIN_SAMPLES', '150'))
    )
    parser.add_argument(
        '--enable-challenger-opt',
        action='store_true',
        default=os.getenv(
            'ENABLE_CHALLENGER_OPTIMIZATION', '0'
        ).lower() in ('1', 'true', 'yes', 'on')
    )
    parser.add_argument(
        '--enable-production-promote',
        action='store_true',
        default=os.getenv(
            'ENABLE_PRODUCTION_PROMOTE', '0'
        ).lower() in ('1', 'true', 'yes', 'on')
    )
    parser.add_argument(
        '--min-samples-chall',
        type=int,
        default=int(os.getenv('CHALL_MIN_SAMPLES', '50'))
    )
    parser.add_argument(
        '--improvement-threshold',
        type=float,
        default=float(os.getenv('CHALL_IMPROVEMENT_THRESHOLD', '0.02'))
    )
