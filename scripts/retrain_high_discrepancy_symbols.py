#!/usr/bin/env python3
"""
Farkların çok olduğu sembolleri tespit edip, study dosyalarından best params ile
düzeltilmiş filtre (low support gating) ile tekrar eğitir.

Kullanım:
    /opt/bist-pattern/venv/bin/python3 scripts/retrain_high_discrepancy_symbols.py --threshold 30.0
"""
...
try:
    ...
    if os.path.exists(venv_python):
        print(f"⚠️ Optuna not found, trying venv python: {venv_python}", file=sys.stderr)
...
if 'DATABASE_URL' not in os.environ:
    ...
    if secret_file.exists():
        ...
        print("⚠️ Warning: DATABASE_URL not set and secret file not found", file=sys.stderr)
else:
...
def find_best_trial_with_filter_applied(db_file: Path, symbol: str, horizon: int,
    ...
    try:
        ...
        for trial in study.trials:
            ...
            # ✅ FIX: Need at least 2 splits for reliable DirHit calculation
...
def get_best_params_from_study(db_file: Path, symbol: str, horizon: int, 
                                use_filtered: bool = True, cycle: Optional[int] = None) -> Optional[Dict]:
    ...
    try:
        ...
        if hasattr(study.best_trial, 'user_attrs'):
            features_enabled = study.best_trial.user_attrs.get('features_enabled', {})
        
        # Get feature_params from best trial user_attrs if available
        feature_params = {}
        if hasattr(study.best_trial, 'user_attrs'):
            feature_params = study.best_trial.user_attrs.get('feature_params', {})
        
        # ✅ NEW: Get filter info from split_metrics
        filter_used = {'min_mask_count': None, 'min_mask_pct': None}
        split_analysis = {'total_splits': 0, 'included_splits': 0, 'excluded_splits': 0}
        ...
        try:
            ...
            if symbol_key in symbol_metrics:
                ...
                if len(min_mask_count_values) == 1:
                    filter_used['min_mask_count'] = list(min_mask_count_values)[0]
                if len(min_mask_pct_values) == 1:
                    filter_used['min_mask_pct'] = list(min_mask_pct_values)[0]
        except Exception:
            pass  # Filter info not critical
        
        # ✅ CRITICAL: If use_filtered=True, find best trial AFTER applying filter
        ...
        if use_filtered and filter_used['min_mask_count'] is not None:
            ...
            min_pct = filter_used['min_mask_pct'] if filter_used['min_mask_pct'] is not None else 0.0
            
            if min_count > 0 or min_pct > 0.0:
                print(f"   🔍 Finding best trial with filter applied (min_count={min_count}, min_pct={min_pct})...")
                filtered_trial, filtered_score = find_best_trial_with_filter_applied(
                    db_file, symbol, horizon, min_count, min_pct
                )
                
                if filtered_trial:
                    print(f"   ✅ Found filtered best trial: #{filtered_trial.number} (filtered DirHit: {filtered_score:.2f}%)")
                    print(f"   📊 Original best trial: #{best_trial_number} (original score: {best_value:.4f})")
                    ...
                    # ✅ CRITICAL: Update JSON file with filtered trial info for consistency
                    # This ensures future retraining uses the correct best params
                    json_file = find_hpo_json(symbol, horizon, cycle)
                    if json_file:
                        print(f"   📝 Updating JSON file with filtered best trial: {json_file.name}")
                        filtered_trial_data = {
        ...
        print(f"❌ Error loading study {db_file}: {e}", file=sys.stderr)
        return None
...
def find_hpo_json(symbol: str, horizon: int, cycle: Optional[int] = None) -> Optional[Path]:
    ...
    if cycle:
        ...
        pattern = f"optuna_pilot_features_on_h{horizon}_c{cycle}_*{symbol}*.json"
...
def update_hpo_json_with_filtered_trial(json_file: Path, filtered_trial_data: Dict) -> bool:
    ...
    try:
        ...
        data['best_dirhit'] = filtered_trial_data.get('best_dirhit', filtered_trial_data['best_value'])
        data['best_params'] = filtered_trial_data['best_params']
        
        # Update best_trial dict if exists
        if 'best_trial' in data:
            data['best_trial']['number'] = filtered_trial_data['best_trial_number']
            data['best_trial']['value'] = float(filtered_trial_data['best_value'])
        ...
        data['filter_applied'] = {
            'min_mask_count': filtered_trial_data.get('filter_used', {}).get('min_mask_count'),
            'min_mask_pct': filtered_trial_data.get('filter_used', {}).get('min_mask_pct'),
        ...
        print(f"❌ Error updating JSON file {json_file}: {e}", file=sys.stderr)
        import traceback
        print(traceback.format_exc(), file=sys.stderr)
        return False


def parse_discrepancy_data(discrepancy_text: str) -> List[Tuple[str, int, float, float, float]]:
    ...
    for line in discrepancy_text.strip().split('\n'):
        ...
        try:
            ...
            if hpo_dirhit is not None and training_dirhit is not None:
                ...
                discrepancies.append((symbol, horizon, hpo_dirhit, training_dirhit, diff))
        except Exception as e:
            print(f"⚠️ Error parsing line '{line}': {e}", file=sys.stderr)
            continue
    
    return discrepancies


def retrain_symbol(symbol: str, horizon: int, best_params_data: Dict, pipeline: ContinuousHPOPipeline) -> bool:
    ...
    try:
        ...
        if hpo_result['features_enabled']:
            best_params_with_metadata['features_enabled'] = hpo_result['features_enabled']
        if hpo_result['feature_params']:
            best_params_with_metadata['feature_params'] = hpo_result['feature_params']
        
        # ✅ CRITICAL: Use the SAME filter that HPO used when finding best params
        ...
        # This way we can see if the discrepancy is due to filter mismatch or other issues
        
        # Filter values are already set in environment from get_best_params_from_study
        # They will be used by _evaluate_training_dirhits in continuous_hpo_training_pipeline.py
        print(f"   🔧 Training will use filter: HPO_MIN_MASK_COUNT={os.environ.get('HPO_MIN_MASK_COUNT', '0')}, "
              f"HPO_MIN_MASK_PCT={os.environ.get('HPO_MIN_MASK_PCT', '0.0')}")
        
        result = pipeline.run_training(symbol, horizon, best_params_with_metadata, hpo_result=hpo_result)
        ...
        print(f"✅ {symbol} {horizon}d: Training completed - WFV DirHit={wfv_dirhit:.2f}%" if wfv_dirhit else f"✅ {symbol} {horizon}d: Training completed")
        
        return True
    except Exception as e:
        print(f"❌ {symbol} {horizon}d: Error during retraining: {e}", file=sys.stderr)
        import traceback
        print(traceback.format_exc(), file=sys.stderr)
        return False
...
def main():
    parser = argparse.ArgumentParser(description='Retrain symbols with high HPO-Training discrepancy')
   ...
   EBEBK_1d: HPO DirHit=80.00% Training DirHit=50.00%
    ...
    # Filter by threshold
    high_discrepancies = [d for d in discrepancies if d[4] >= args.threshold]
    
    # Filter by symbols if specified
    if args.symbols:
        high_discrepancies = [d for d in high_discrepancies if d[0] in args.symbols]
    
    # Filter by horizons if specified
    if args.horizons:
        high_discrepancies = [d for d in high_discrepancies if d[1] in args.horizons]
    ...
    print("=" * 80)
    print(f"📊 Found {len(high_discrepancies)} symbols with discrepancy >= {args.threshold}%")
    print()
    ...
    for symbol, horizon, hpo_dirhit, train_dirhit, diff in high_discrepancies:
        print(f"   {symbol}_{horizon}d: HPO={hpo_dirhit:.2f}% → Training={train_dirhit:.2f}% (diff={diff:.2f}%)")
    print()
    ...
    with app.app_context():
        for symbol, horizon, hpo_dirhit, train_dirhit, diff in high_discrepancies:
            ...
            if not study_db:
                print(f"⚠️ {symbol} {horizon}d: Study DB not found, trying to find HPO JSON...")
                ...
                if hpo_json:
                    ...
                    try:
                        ...
                        print(f"❌ Error loading HPO JSON: {e}", file=sys.stderr)
                        fail_count += 1
                else:
                    print(f"❌ {symbol} {horizon}d: Neither study DB nor HPO JSON found")
                    fail_count += 1
                continue
            ...
            best_params_data = get_best_params_from_study(study_db, symbol, horizon, use_filtered=True, cycle=cycle)
            if not best_params_data:
                print(f"❌ {symbol} {horizon}d: Could not load best params from study")
                fail_count += 1
                continue
            ...
            print(f"✅ Best trial: {best_params_data.get('best_trial_number')}, Best value: {best_params_data.get('best_value', 0):.2f}")
            print(f"   Filter used: min_count={filter_info.get('min_mask_count', 'N/A')}, min_pct={filter_info.get('min_mask_pct', 'N/A')}")
            print(f"   Splits: {split_info.get('included_splits', 0)}/{split_info.get('total_splits', 0)} included")
            
            # ⚠️  Warning if best params found with limited splits
            if split_info.get('excluded_splits', 0) > 0:
                print(f"   ⚠️  WARNING: {split_info.get('excluded_splits')} split(s) excluded - best params may not be optimal!")
            
            # ✅ CRITICAL: Set filter for retraining to EXACTLY match what HPO used
            # This ensures training evaluation uses the same filter as HPO did
            # If HPO excluded low support splits, training should also exclude them
            # This way we test if best params are truly optimal for the filtered splits
            ...
            print(f"   🔧 Training will use HPO's filter: HPO_MIN_MASK_COUNT={os.environ.get('HPO_MIN_MASK_COUNT')}, "
