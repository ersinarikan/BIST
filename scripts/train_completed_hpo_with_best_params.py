#!/usr/bin/env python3
"""
Train completed HPO symbols with best parameters (all features ON) and
evaluate DirHit
This script:
...
# Skip PredictionsLog writes during training
os.environ['DISABLE_PREDICTIONS_LOG'] = '1'
# Disable ML prediction during training
os.environ['DISABLE_ML_PREDICTION_DURING_TRAINING'] = '1'
# Disable prediction write during training
            ...
            with open(secret_path, 'r') as sp:
                ...
                if _pwd:
                    os.environ['DATABASE_URL'] = (
                        f'postgresql://bist_user:{_pwd}@127.0.0.1:6432/'
                        f'bist_pattern_db'
                    )
        if 'DATABASE_URL' not in os.environ:
...
def set_hpo_params_as_env(params: dict, horizon: int):
    ...
    for key, val in params.items():
        if key.startswith('xgb_'):
            ...
            if key == 'cat_bootstrap_type':
                ...
                # ✅ FIX: Only set subsample if bootstrap_type is not
                # 'Bayesian' (unsupported) and not 'No' (bootstrap disabled)
                # When bootstrap_type='No' or 'Bayesian', subsample should
                # not be set
                # Check both from params dict and environment (in case
                # bootstrap_type was set earlier)
                bt_check = (
                    bootstrap_type_value
                    or os.environ.get('OPTUNA_CAT_BOOTSTRAP_TYPE', '').strip()
                )
                bt_check_lower = bt_check.lower() if bt_check else ''
                # Exclude both 'Bayesian' (doesn't support subsample) and 'No'
                # (bootstrap disabled)
                if bt_check_lower not in (
                    'bayesian', 'no', 'false', '0', 'none', ''
                ):
                    os.environ[env_key] = str(val)
                    cat_params.append(
                        f"{key}={val:.4f}" if isinstance(val, float)
                    ...
                    # Skip subsample when bootstrap is disabled or Bayesian
                    # (unsupported)
                    continue
            else:
                os.environ[env_key] = str(val)
    ...
    # Propagate feature parameters (smart ensemble, directional loss,
    # externals) if present
    feature_params = {}
    try:
    ...
    if xgb_params or lgb_params or cat_params:
        logger.debug(
            f"   ⚙️ Best HPO parameters set for {horizon}d: "
            f"XGB({len(xgb_params)}), LGB({len(lgb_params)}), "
            f"CAT({len(cat_params)})"
        )


def train_symbol_with_all_horizons_best_params(
    symbol: str, all_horizon_params: dict
) -> dict:
    ...
    NOTE: Since enhanced_ml_system.py reads OPTUNA_XGB_* globally (not
    per-horizon), we need to train each horizon separately with its own
    parameters.
    """
    try:
        ...
        logger.info(
            f"🎯 Training {symbol} for horizons {horizons_list} with best "
            f"HPO parameters..."
        )
        logger.info(
            "   ⚠️  Note: Training each horizon separately to use correct "
            "parameters"
        )
        ...
        with app.app_context():
            ...
            if df is None or len(df) < 50:
                logger.warning(
                    ...
                    f"({len(df) if df is not None else 0} days)"
                )
            ...
            for horizon in horizons_list:
                ...
                # ⚡ CRITICAL: Set only this horizon for training BEFORE
                # creating instance
                # DO NOT set ML_HORIZONS to all horizons - it will be cached!
                os.environ['ML_HORIZONS'] = str(horizon)

                logger.info(
                    f"   🎯 Training {symbol} {horizon}d with best "
                    f"parameters..."
                )

                # ⚡ CRITICAL: Clear ConfigManager cache AND remove from
                # cache before creating new instance
                # ConfigManager caches environment variables, so we need to
                # clear cache when ML_HORIZONS changes. Also remove
                # 'ML_HORIZONS' from cache specifically.
                try:
                    from bist_pattern.core.config_manager import (
                        ConfigManager
                    )
                    ConfigManager.clear_cache()  # Clear all cache
                    # Also remove ML_HORIZONS from cache specifically (in
                    # case clear_cache doesn't work)
                    if (
                        hasattr(ConfigManager, '_cache')
                        and 'ML_HORIZONS' in ConfigManager._cache
                    ):
                        del ConfigManager._cache['ML_HORIZONS']
                except Exception as e:
                    logger.debug(
                        f"Failed to clear ML_HORIZONS cache (fallback): {e}"
                    )

                # ⚡ CRITICAL: Reset singleton instance to read new
                # ML_HORIZONS
                # Singleton instance's prediction_horizons is set in __init__
                # based on ML_HORIZONS
                ...
                # Get fresh instance to read new env vars
                ml = get_enhanced_ml_system()
                # Verify prediction_horizons is correct
                if ml.prediction_horizons != [horizon]:
                    logger.warning(
                        f"   ⚠️  {symbol} {horizon}d: prediction_horizons "
                        f"mismatch! Expected [{horizon}], got "
                        f"{ml.prediction_horizons}"
                    )
                    # Force update - this is safe because we're training only
                    # this horizon
                    ml.prediction_horizons = [horizon]
                    logger.info(
                        f"   ✅ {symbol} {horizon}d: prediction_horizons "
                        f"force-updated to [{horizon}]"
                    )
                ...
                if not result:
                    logger.warning(
                        f"   ❌ {symbol} {horizon}d: Training failed"
                    )
                    all_success = False
                else:
                    logger.info(
                    ...
                    # ⚡ CRITICAL FIX: Ensure features are saved to disk
                    # after each horizon training
                    # This ensures that when prediction is done later, all
                    # horizon features are available
                    try:
                        ml.save_enhanced_models(symbol)
                        logger.debug(
                            f"   ✅ {symbol} {horizon}d: Features saved to "
                            f"disk"
                        )
                    except Exception as e:
                        logger.warning(
                            f"   ⚠️  {symbol} {horizon}d: Failed to save "
                            f"features: {e}"
                        )

            # ⚡ CRITICAL FIX: After all horizons are trained, load all
            # features into a single instance
            # This ensures prediction can access all horizon features
            # Restore all horizons for final status
            os.environ['ML_HORIZONS'] = ','.join(
                str(h) for h in horizons_list
            )
            ...
            try:
                # ⚡ CRITICAL FIX: Use load_trained_models() instead of
                # _load_models_from_disk()
                # _load_models_from_disk() method doesn't exist, use
                # load_trained_models() instead
                if hasattr(ml_final, 'load_trained_models'):
                    ml_final.load_trained_models(symbol)
                    logger.debug(
                        f"✅ {symbol}: All horizon features loaded from disk"
                    )
                else:
                    # Fallback: manually load horizon_features.json
                    import json
                    horizon_cols_file = (
                        f"{ml_final.model_directory}/"
                        f"{symbol}_horizon_features.json"
                    )
                    if os.path.exists(horizon_cols_file):
                        ...
                        for h in horizons_list:
                            ...
                            if h_key in horizon_features:
                                ...
                                ml_final.models[feature_key] = (
                                    horizon_features[h_key]
                                )
                        logger.debug(
                            f"✅ {symbol}: Horizon features loaded from disk "
                            f"({len(horizon_features)} horizons)"
                        )
                    else:
                        logger.warning(
                            f"⚠️  {symbol}: Horizon features file not "
                            f"found: {horizon_cols_file}"
                        )
            except Exception as e:
                logger.warning(
                    f"⚠️  {symbol}: Failed to load horizon features from "
                    f"disk: {e}"
                )

            if all_success:
                logger.info(
                    f"✅ {symbol}: Training completed successfully for all "
                    f"horizons {horizons_list} with best HPO parameters"
                )

                # ⚡ CRITICAL: Evaluate DirHit using walk-forward validation
                logger.info(
                    f"📊 Evaluating DirHit for {symbol} with walk-forward "
                    f"validation..."
                )
                dirhit_results = evaluate_dirhit_walkforward(
                    symbol, df, horizons_list, all_horizon_params, ml_final
                )
...
def evaluate_dirhit_walkforward(
    symbol: str,
    df: pd.DataFrame,
    horizons_list: list,
    all_horizon_params: dict,
    ml_final
) -> dict:
    """Evaluate DirHit using walk-forward validation for all horizons.

    ⚡ CRITICAL: Disable adaptive learning during validation to prevent
    data leakage.
    - Training: Adaptive learning ON (model learns from test_data)
    - Validation: Adaptive learning OFF (model only predicts, no updates)
    This ensures validation results are realistic and no data leakage
    occurs.
    """
    dirhit_results = {}
    ...
    # Validation'da adaptive learning kapalı olmalı (data leakage önlemek
    # için)
    original_adaptive = os.environ.get('ML_USE_ADAPTIVE_LEARNING', '0')
    os.environ['ML_USE_ADAPTIVE_LEARNING'] = '0'
    logger.debug(
        f"   🔒 Adaptive learning disabled for validation "
        f"(was: {original_adaptive})"
    )
    ...
    try:
        ...
        if total_days < 180:
            logger.warning(
                f"⚠️ {symbol}: Insufficient data for walk-forward "
                f"validation ({total_days} days, need at least 180)"
            )
        ...
        for horizon in horizons_list:
            try:
                ...
                if len(test_df) < min_test_days:
                    logger.warning(
                        f"   ⚠️ {symbol} {horizon}d: Insufficient test data "
                        f"({len(test_df)} days, need {min_test_days})"
                    )
                    continue
                ...
                for t in range(len(test_df) - horizon):
                    try:
                        ...
                        logger.debug(
                            f"   {symbol} {horizon}d: Prediction error at "
                            f"t={t}: {e}"
                        )
                        continue
                ...
                if valid_predictions > 0:
                    ...
                    logger.info(
                        f"   ✅ {symbol} {horizon}d: DirHit = {dh:.2f}% "
                        f"({valid_predictions} valid predictions)"
                    )
                else:
                    logger.warning(
                        f"   ⚠️ {symbol} {horizon}d: No valid predictions"
                    )
                    dirhit_results[horizon] = {
                        'dirhit': None,
                        'valid_predictions': 0,
                ...
                logger.error(
                    f"   ❌ {symbol} {horizon}d: DirHit evaluation failed: "
                    f"{e}"
                )
                import traceback
                logger.debug(traceback.format_exc())
        ...
        logger.error(
            f"   ❌ {symbol}: Walk-forward validation failed: {e}"
        )
        import traceback
        logger.debug(traceback.format_exc())
    finally:
        ...
        logger.debug(
            f"   🔓 Adaptive learning restored to: {original_adaptive}"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Train completed HPO symbols with best parameters'
    )
    parser.add_argument(
        '--phase',
        type=str,
    ...
    if args.phase == 'phase2':
        completed_file = (
            '/opt/bist-pattern/results/'
            'completed_hpo_symbols_phase2.json'
        )
    ...
    if not os.path.exists(completed_file):
        logger.error(
            f"Completed HPO symbols file not found: {completed_file}"
        )
        return 1
    ...
    logger.info("=" * 80)
    logger.info(
        f"🎯 Training Completed HPO Symbols with Best Parameters "
        f"({args.phase.upper()})"
    )
    logger.info("=" * 80)
    logger.info("All features enabled (production-like training)")
    logger.info(
        f"Loaded {sum(len(items) for items in completed.values())} "
        f"symbol/horizon pairs"
    )
    logger.info("=" * 80)
    ...
    # ⚡ CRITICAL: For Phase 2, only process the 5 symbols used in Phase 2
    # HPO
    ...
    if args.phase == 'phase2':
        ...
        symbol_horizons = {
            sym: data
            for sym, data in symbol_horizons.items()
            if sym in phase2_symbols
        }
        logger.info(
            f"🔍 Phase 2: Filtering to {len(symbol_horizons)} symbols: "
            f"{sorted(symbol_horizons.keys())}"
        )

    for symbol, horizons_data in symbol_horizons.items():
        # Extract best_params for each horizon
        all_horizon_params = {
            h: data['best_params'] for h, data in horizons_data.items()
        }

        logger.info(
            f"\n🎯 Processing {symbol} for horizons "
            f"{sorted(all_horizon_params.keys())}..."
        )

        result = train_symbol_with_all_horizons_best_params(
            symbol, all_horizon_params
        )
        ...
        if success:
            ...
            for horizon, data in horizons_data.items():
                ...
                results.append({
                    ...
                    'valid_predictions': horizon_dirhit.get(
                        'valid_predictions'
                    ),
    ...
    successful_results = [
        r for r in results
        if r.get('status') == 'success' and r.get('dirhit') is not None
    ]
    if successful_results:
        avg_dirhit = np.mean([r['dirhit'] for r in successful_results])
        logger.info("=" * 80)
        logger.info(
            f"📊 Average DirHit: {avg_dirhit:.2f}% "
            f"({len(successful_results)} symbol/horizon pairs)"
        )
        logger.info("=" * 80)
    ...
    logger.info("=" * 80)
    logger.info(
        f"✅ Training completed: {success_count} success, {fail_count} "
        f"failed"
    )
