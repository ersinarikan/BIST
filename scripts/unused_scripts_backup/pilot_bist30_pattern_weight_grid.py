#!/usr/bin/env python3
"""
Pattern weight scale grid (DB-backed) for short horizons 1/3/7d.

Grids ML_PATTERN_WEIGHT_SCALE_{1D,3D,7D} over [0.8, 1.0, 1.2, 1.4] and records
best metrics per symbol/horizon optimizing DirHit first, then nRMSE.

Outputs CSV: symbol,horizon,best_scale,r2,dir_hit_pct,mape,nrmse
"""
...
def run_grid() -> None:
    ...
    for sym in BIST30:
        ...
        for hz in ['1d', '3d', '7d']:
            ...
            for sc in scales:
                os.environ['ML_PATTERN_WEIGHT_SCALE_1D'] = str(sc if hnum == 1 else os.getenv('ML_PATTERN_WEIGHT_SCALE_1D', '1.0'))
                os.environ['ML_PATTERN_WEIGHT_SCALE_3D'] = str(sc if hnum == 3 else os.getenv('ML_PATTERN_WEIGHT_SCALE_3D', '1.0'))
                os.environ['ML_PATTERN_WEIGHT_SCALE_7D'] = str(sc if hnum == 7 else os.getenv('ML_PATTERN_WEIGHT_SCALE_7D', '1.0'))
                ...
                if not m:
                    continue
                sc_tuple = score_tuple(m)
                if sc_tuple > best_score:
                    best_score = sc_tuple
                    best_scale = sc
                    best_metrics = {
                        'r2': m.get('r2'),
    ...
    if not rows:
        print('⚠️ No pattern-weight grid results.')
        return

    out_dir = os.getenv('BIST_LOG_PATH', '/opt/bist-pattern/logs')
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, f"bist30_pattern_weight_grid_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")
    pd.DataFrame(rows).to_csv(out_path, index=False)
    print(f"✅ Wrote pattern-weight grid summary: {out_path}")
