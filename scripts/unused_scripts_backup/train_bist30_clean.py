#!/usr/bin/env python3
"""
BIST30 Clean Training - Hatasız eğitim, detaylı raporlama
Backtest devre dışı, sadece model eğitimi
"""
import os
import sys
import logging
...
def main():
    ...
    logger.info("=" * 100)
    logger.info("BIST30 CLEAN TRAINING - HATASIZ EĞİTİM")
    logger.info("=" * 100)
    ...
    for i, sym in enumerate(BIST30_SYMBOLS, 1):
        ...
        try:
            ...
            if success:
                ...
                logger.error(f"❌ Başarısız (training returned False)")
    ...
    results_file = f'/opt/bist-pattern/logs/bist30_training_results_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
