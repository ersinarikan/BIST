#!/usr/bin/env python3
"""
Directional-loss ON pilot for short horizons (1/3/7d) on BIST30 (DB-backed).

Outputs CSV: symbol,horizon,r2,dir_hit_pct,mape,nrmse
"""
...
def run_pilot() -> None:
    ...
    os.environ['ML_USE_STACKED_SHORT'] = '0'  # regressor path remains for magnitude
    ...
    out_path = os.path.join(out_dir, f"bist30_dirloss_short_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")
    pd.DataFrame(rows).to_csv(out_path, index=False)
    print(f"✅ Wrote dirloss short summary: {out_path}")
