...
class AsyncRSSNewsProvider:
    ...
    def __init__(self, worker_threads: int = 2):
        ...
        self._background_pool = ThreadPoolExecutor(
            max_workers=1, thread_name_prefix="RSSFetcher"
        )
        ...
        self.financial_keywords = [
            ...
            'yatırım', 'kar', 'zarar', 'büyüme', 'resesyon', 'ihracat',
            'ithalat'
        ]
        ...
        logger.info(
            f"📰 Async RSS News Provider initialized "
            f"({len(self.rss_sources)} sources)"
        )
    ...
    def _start_background_fetching(self):
                ...
                try:
                    ...
                    if loop.is_running():
                        ...
                        def _run_in_isolated_thread(coro):
                            """Run coroutine in a completely isolated thread
                            with new loop"""
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    logger.debug(
                                        f"Failed to close new_loop: {e}"
                                    )
                        with concurrent.futures.ThreadPoolExecutor() as (
                            executor
                        ):
                            future = executor.submit(
                                _run_in_isolated_thread, coro
                            )
        ...
        def _run_in_thread_with_loop(coro):
            ...
            try:
                ...
                try:
                    ...
                    logger.debug(
                        f"Failed to close loop in _run_in_thread_with_loop: "
                        f"{e}"
                    )
        ...
        def background_fetcher():
            ...
            while True:
                try:
                    ...
                    # 5 minutes
                    if (
                        current_time - self._last_fetch_time > 300 and
                        not self._fetching
                    ):
                        # ✅ FIX: Use ThreadPoolExecutor for thread-safe
                        # execution
                        import concurrent.futures

                        def _run_in_isolated_thread():
                            """Run in completely isolated thread with new
                            loop"""
                            loop = asyncio.new_event_loop()
                            asyncio.set_event_loop(loop)
                            try:
                                return loop.run_until_complete(
                                    self._fetch_all_feeds_async()
                                )
                            finally:
                                try:
                                    pending = asyncio.all_tasks(loop)
                                    ...
                                    logger.debug(
                                        f"Failed to close loop in executor: "
                                        f"{e}"
                                    )
        ...
        thread = threading.Thread(
            target=background_fetcher, daemon=True,
            name="RSSBackgroundFetcher"
        )
        thread.start()
            ...
            def _run_in_isolated_thread():
                ...
                try:
                    ...
                    try:
                        ...
                        logger.debug(
                            f"Failed to close loop before executor: {e}"
                        )
            with concurrent.futures.ThreadPoolExecutor(
                max_workers=1
            ) as executor:
    ...
    async def _create_session(self) -> aiohttp.ClientSession:
        ...
        headers = {
            'User-Agent': (
                'Mozilla/5.0 (compatible; BIST-Pattern RSS Reader; '
                '+https://github.com/bist-pattern)'
            )
        }
    ...
    async def _fetch_all_feeds_async(self):
        ...
        try:
            ...
            logger.debug(
                f"🔄 Background fetching {len(self.rss_sources)} RSS feeds..."
            )

            async with await self._create_session() as session:
                # Fetch all feeds concurrently
                tasks = [
                    self._fetch_single_feed(session, url)
                ...
                for i, result in enumerate(feed_results):
                    if isinstance(result, Exception):
                        logger.warning(
                            f"RSS feed error {self.rss_sources[i]}: {result}"
                        )
                ...
                self._stats['last_fetch_duration'] = int(
                    time.time() - fetch_start_time
                )

                logger.info(
                    f"✅ RSS fetch completed: {len(all_news_items)} news items "
                    f"cached in {self._stats['last_fetch_duration']:.1f}s"
                )
        ...
        try:
            ...
            for item in items[:self.max_items]:
                try:
                    ...
                    # Extract description
                    desc_elem = (
                        item.find('description') or item.find('summary')
                    )
                    ...
                    if date_elem is not None and date_elem.text:
                        ...
                        # ✅ FIX: Parse pub_date to timestamp for
                        # lookback_hours filtering
                        try:
                            ...
                            try:
                                ...
                                logger.debug(
                                    f"Failed to parse pub_date with "
                                    f"parsedate_to_datetime: {e}"
                                )
                            ...
                            logger.debug(
                                f"Failed to parse pub_date completely: {e}"
                            )
                    ...
                    if text and len(text) > 20:  # Minimum text length
                        news_items.append({
                            ...
                            # ✅ FIX: Store timestamp for filtering
            ...
            with self._cache_lock:
                ...
                if 'global_news' in self._news_cache:
                    ...
                    if (
                        ...
                        # ✅ STRICT FIX: Filter news relevant to symbol
                        # (symbol-specific only)
                        symbol_upper = symbol.upper()
                        relevant_news = []

                        for news_item in all_news:
                            # ✅ FIX: Filter by lookback_hours (24 hours)
                            # based on pub_timestamp
                            pub_timestamp = news_item.get('pub_timestamp')
                            # 🚫 Legacy entries without pub_timestamp must
                            # NOT bypass age filtering
                            if not isinstance(pub_timestamp, (int, float)):
                                # Skip items with unknown publish time
                                continue
                            age_hours = (
                                time.time() - float(pub_timestamp)
                            ) / 3600.0
                            if age_hours > self.lookback_hours:
                                # Skip news older than lookback_hours
                                continue
                        ...
                        if relevant_news:
                            ...
                            logger.debug(
                                f"📰 Found {len(relevant_news)} specific news "
                                f"for {symbol}"
                            )
                        else:
                            # ✅ DEBUG: Log sample news items to understand
                            # why no match (INFO level for visibility)
                            if all_news and len(all_news) > 0:
                                sample_count = min(3, len(all_news))
                                logger.info(
                                    f"📰 {symbol}: No specific news found. "
                                    f"Cache has {len(all_news)} total news. "
                                    f"Sample titles:"
                                )
                                for i, news_item in enumerate(
                                    all_news[:sample_count], 1
                                ):
                                    title = news_item.get(
                                        'title', news_item.get('text', '')
                                    )[:100]
    ...
    def _is_relevant_news(self, text: str, symbol: str) -> bool:
        """Check if news text is relevant to the symbol (symbol name or
        company name)"""
        if not text:
            return False
        ...
        AMBIGUOUS_SYMBOLS = {
            'HEDEF': [
                'HEDEF GYO', 'HEDEF GAYR', 'HEDEF.IS', 'HEDEF HOLD',
                'HEDEF HİSSE'
            ],
        ...
        # ✅ FIX: Check special cases BEFORE database lookup (works even
        # without DB)
        ...
        special_cases = {
            ...
            'AKBNK': ['AKBANK'],
            # ✅ FIX: ASELSAN is the company name for ASELS
            'ASELS': ['ASELSAN'],
            'BIMAS': ['BİM', 'BIM'],
            'TUPRS': ['TÜPRAŞ', 'TUPRAS'],
            'PETKM': ['PETKİM', 'PETKIM'],
            ...
            for special_name in special_names:
                if special_name and len(special_name) > 2:
                    ...
                    if any(
                        ...
                        return True
                    # Also check if special_name is a substring (for compound
                    # words, minimum 4 chars)
        ...
        try:
            ...
            try:
                # Flask's _get_current_object is a private method
                ...
                with app_obj.app_context():
                    ...
                    stock = (
                        Stock.query.filter_by(symbol=symbol_upper).first()
                    )
                    if stock and stock.name:
                        company_name = stock.name.upper()

                        # Create multiple variants of company name for
                        # flexible matching
                        name_variants = []
                        ...
                        suffixes = [
                            ' A.Ş.', ' A.S.', ' T.A.Ş.', ' T.A.S.',
                            ' A.O.', ' A.O.', ' ŞİRKETİ', ' ŞIRKETI',
                            ' HOLDİNG', ' HOLDING', ' SANAYİ', ' SANAYI',
                            ' VE TİCARET', ' VE TICARET'
                        ]
                        for suffix in suffixes:
                            base_name = base_name.replace(suffix, '').strip()
                        name_variants.append(base_name)

                        # Extract key words (first 2-3 words, usually
                        # company name)
                        words = base_name.split()
                        if len(words) >= 2:
                            # First 2 words (e.g., "ASELSAN ELEKTRONİK"
                            # from "ASELSAN ELEKTRONİK SANAYİ VE TİCARET")
                        ...
                        for variant in name_variants:
                            if variant and len(variant) > 2:
                                # Flexible matching: word boundaries,
                                # punctuation, etc.
                                variant_patterns = [
                                    f" {variant} ",  # Space before and after
                                    f" {variant},",  # Space before, comma
                                    f" {variant}.",  # Space before, period
                                    f" {variant}:",  # Space before, colon
                                    f" {variant}-",  # Space before, dash
                                ...
                                # Check at start/end
                                if (
                                    text_upper.startswith(variant + " ") or
                                    text_upper.endswith(" " + variant)
                                ):
                                    return True

                                # Also check if variant is a substring (for
                                # compound words, minimum 4 chars)
            ...
            def safe_async_run():
                try:
                    ...
                    if (
                        ...
                        logger.debug(
                            "RSS: Interpreter shutdown, "
                            "skipping background fetch"
                        )
                    else:
        ...
        with self._cache_lock:
            ...
            if 'global_news' in self._news_cache:
                ...
                cache_info = {
                    ...
                    'cache_age_seconds': (
                        time.time() - cache_entry.get('timestamp', 0)
                    ),
                    'cache_fresh': (
                        time.time() - cache_entry.get('timestamp', 0) <
                        self.cache_ttl
    ...
    def cleanup_old_cache(self, max_age_seconds: int = 3600):
        ...
        if to_remove:
            logger.debug(
                f"🧹 Cleaned up {len(to_remove)} old RSS cache entries"
            )
