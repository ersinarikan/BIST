...
try:
    ...
    FINBERT_AVAILABLE = False
    logger.warning(
        "FinBERT dependencies not available. "
...
class FinGPTAnalyzer:
    ...
    def __init__(self):
        ...
        if FINBERT_AVAILABLE:
            try:
                ...
                import os
                # ✅ FIX: Set TRANSFORMERS_CACHE and HF_HOME before
                # loading model
                cache_dir = os.getenv(
                    'TRANSFORMERS_CACHE',
                    '/opt/bist-pattern/.cache/huggingface'
                )
                ...
                use_turkish_model = (
                    os.getenv('USE_TURKISH_SENTIMENT', 'True').lower() ==
                    'true'
                )
                ...
                if use_turkish_model:
                    ...
                    logger.info(
                        f"🇹🇷 Türkçe sentiment modeli yükleniyor "
                        f"(cache: {cache_dir})..."
                    )
                    ...
                    try:
                        ...
                        logger.info(
                            f"📥 Türkçe model cache'de bulunamadı "
                            f"({cache_err}), network'ten indiriliyor..."
                        )
                        self.tokenizer = AutoTokenizer.from_pretrained(
                ...
                logger.info(
                    f"✅ Sentiment modeli başarıyla yüklendi: {model_name}"
                )

            except Exception as e:
                logger.warning(
                    f"⚠️ Türkçe model yükleme hatası, "
                    f"FinBERT'e geçiliyor: {e}"
                )
                # Fallback to FinBERT
                try:
                    model_name = "ProsusAI/finbert"
    ...
    def analyze_sentiment(self, text):
        ...
        try:
            ...
            if (
                ...
                labels = [
                    str(label_value).lower() for label_value in raw_labels
                ]
            else:
            ...
            if len(scores) >= 3:
                ...
                # Etiketler 'positive'/'negative' olmayabilir, bu yüzden en
                # yüksek iki skoru pozitif/negatif olarak eşleştir
        ...
        try:
            if not news_texts:
                try:
                    self._broadcast(
                        'INFO',
                        f"FinGPT {symbol}: news=0 overall=neutral "
                        f"conf=0.00 (no_news)",
                        'news'
                    )
                    try:
            ...
            positive_count = sum(
                ...
                if str(s.get('sentiment', '')).lower() == 'positive'
            )
            negative_count = sum(
                1 for s in sentiments
                if str(s.get('sentiment', '')).lower() == 'negative'
            )
            neutral_count = sum(
                1 for s in sentiments
                if str(s.get('sentiment', '')).lower() == 'neutral'
            )

            total_positive_score = sum(
                float(s.get('scores', {}).get('positive', 0.0))
                for s in sentiments
            )
            total_negative_score = sum(
                float(s.get('scores', {}).get('negative', 0.0))
                for s in sentiments
            )
            total_neutral_score = sum(
                float(s.get('scores', {}).get('neutral', 0.0))
                for s in sentiments
            )
            ...
            # Dominant sentiment
            if (
                avg_positive > avg_negative and
                avg_positive > avg_neutral
            ):
                overall_sentiment = 'positive'
                overall_confidence = avg_positive
            elif (
                avg_negative > avg_positive and
            ...
            # Broadcast compact log to admin dashboard (toggle with
            # FINGPT_BROADCAST_LOGS)
            try:
                import os as _os
                if str(_os.getenv('FINGPT_BROADCAST_LOGS', '1')).lower() in (
                    '1', 'true', 'yes'
                ):
                    _msg = (
                        f"FinGPT {symbol}: news={len(news_texts)} "
            ...
            logger.error(
                f"Stock news analysis hatası: {e}"
            )
            return {
                'symbol': symbol,
                'overall_sentiment': 'neutral',
    ...
    def _broadcast(
        self, level: str, message: str, category: str = 'news'
    ) -> None:
        try:
            ...
            logger.debug(
                f"Failed to broadcast FinGPT result "
                f"(no app context): {e}"
            )

    def get_sentiment_signal(self, sentiment_result):
        """Sentiment'den trading sinyal türü belirle"""
        # Check if result has status (from analyze_sentiment) or not
        # (from analyze_stock_news)
        if (
            'status' in sentiment_result and
            sentiment_result['status'] != 'success'
        ):
            return 'NEUTRAL'

        # Handle both 'sentiment' and 'overall_sentiment' keys for
        # compatibility
        sentiment = (
            sentiment_result.get('sentiment') or
            sentiment_result.get('overall_sentiment', 'neutral')
        )
        ...
        try:
            ...
            threshold = float(
                ConfigManager.get('FINGPT_CONFIDENCE_THRESHOLD', '0.3')
            )
        except Exception as e:
            logger.debug(
                f"Failed to get FINGPT_CONFIDENCE_THRESHOLD, using 0.3: {e}"
            )
    ...
    def get_system_info(self):
        ...
        return {
            ...
            'model_name': (
                "ProsusAI/finbert" if self.model_loaded else None
            )
        }
