...
from models import (
    db, SimulationSession, SimulationTrade, PortfolioSnapshot, Stock,
    StockPrice
)
...
class SimulationEngine:
    ...
    def create_session(
        self, user_id: int, initial_balance: float = 100000.0,
        duration_hours: int = 48,
        session_name: str = "AI Performance Test"
    ) -> SimulationSession:
    ...
    def process_signal(
        self, session_id: int, symbol: str, signal_data: Dict
        ...
        try:
            ...
            if confidence < min_conf:  # altındaki sinyalleri ignore et
                logger.info(
                    f"📊 Düşük confidence signal ignore edildi: {symbol} - "
                    f"{confidence:.2%}"
                )
                return None
            ...
            if signal_type == 'BULLISH':
                trade = self._execute_buy_signal(
                    session, stock, current_price, signal_data
                )
            elif signal_type == 'BEARISH':
                trade = self._execute_sell_signal(
                    session, stock, current_price, signal_data
                )
            ...
            if trade:
                ...
                self._create_portfolio_snapshot(session)
                logger.info(
                    f"📈 Trade executed: {trade.trade_type} "
                    f"{trade.quantity}x{symbol} @ {current_price}"
                )
    ...
    def _execute_buy_signal(
        self, session: SimulationSession, stock: Stock,
        price: Decimal, signal_data: Dict
    ) -> Optional[SimulationTrade]:
        """Buy signal'i işle"""
        try:
            # Sabit trade amount (parametrik değer)
            # Her alım için sabit miktar
            trade_amount = self.fixed_trade_amount

            if trade_amount < self.min_trade_amount:
                logger.info(
                    f"💰 Trade amount çok küçük: {trade_amount} < "
                    f"{self.min_trade_amount}"
                )
                return None
            ...
            trade = SimulationTrade(
                ...
                signal_source=(
                    ...
                    .get('signals', [{}])[0].get('source', 'UNKNOWN')
                ),
                signal_confidence=Decimal(
                    str(signal_data.get('overall_signal', {})
                        .get('confidence', 0))
                ),
            ...
            logger.info(
                f"💰 {stock.symbol} alındı: {quantity} adet @ {price} = "
                f"{actual_cost} TL"
            )
    ...
    def _execute_sell_signal(
        ...
        try:
            ...
            logger.info(
                f"💰 {stock.symbol} için {len(open_positions)} pozisyon "
                f"satılıyor, toplam: {total_quantity}"
            )

            # Ortalama alış fiyatını hesapla
            total_cost = sum(
                float(pos.total_amount) for pos in open_positions
            )
            # avg_buy_price hesaplandı ancak kullanılmıyor; yalnızca P/L
            # hesaplaması yapılıyor
            ...
            trade.profit_loss = Decimal(
                str(total_profit_loss)
            )

            if total_cost > 0:
                trade.profit_loss_percentage = Decimal(
                    str((total_profit_loss / total_cost) * 100)
                )
            ...
            session.total_trades += 1
            logger.info(
                f"💰 {stock.symbol} satıldı: {quantity} adet @ {price} = "
                f"{net_amount} TL, P/L: {total_profit_loss:.2f}"
            )
            ...
            logger.error(
                f"❌ Sell signal execution hatası: {e}"
            )
            db.session.rollback()
            return None
    ...
    def _get_current_price(self, symbol: str) -> Optional[Decimal]:
        ...
        try:
            ...
            if stock:
                ...
                if latest_price:
                    logger.info(
                        f"📊 Database'den fiyat alındı {symbol}: "
                        f"{latest_price.close_price}"
                    )
            ...
            if not data.empty:
                ...
                logger.info(
                    f"📈 Yahoo Finance'dan fiyat alındı {symbol}: {price}"
                )
                return price
            else:
                logger.warning(
                    f"⚠️ Yahoo Finance'dan veri gelmedi: {ticker_symbol}"
                )
    ...
    def _broadcast_trade_update(
        self, trade: SimulationTrade, stock: Stock
    ):
        ...
        try:
            ...
            trade_data = {
                ...
                'profit_loss': (
                    float(trade.profit_loss) if trade.profit_loss else None
                )
            }
            ...
            logger.info(
                f"📡 Trade broadcast sent: {trade.trade_type} "
                f"{stock.symbol}"
            )
    ...
    def _create_portfolio_snapshot(self, session: SimulationSession):
        ...
        try:
            ...
            total_stocks_value = sum(
                Decimal(str(pos['current_value'])) for pos in open_positions
            )
            ...
            # Sadece hisse değeri
            total_portfolio_value = Decimal(str(total_stocks_value))
            profit_loss = (
                total_portfolio_value - total_investment +
                self._calculate_total_cash_from_sales(session.id)
            )
            profit_loss_percentage = (
                (profit_loss / total_investment) * Decimal('100')
        ...
        for trade in buy_trades:
            ...
            if symbol not in stock_positions:
                stock_positions[symbol] = {
                    'quantity': 0, 'cost_basis': 0, 'stock': trade.stock
                }
    ...
    def _calculate_total_investment(self, session_id: int) -> Decimal:
        ...
        return sum(
            (trade.total_amount for trade in total_bought), Decimal('0')
        )
    ...
    def get_session_performance(self, session_id: int) -> Dict:
        ...
        try:
            ...
            # Portfolio snapshots
            snapshots = (
                PortfolioSnapshot.query.filter_by(session_id=session_id)
