...
class AsyncEnhancedMLSystem:
    ...
    def __init__(self, max_cache_size: int = 100, worker_threads: int = 4):
        ...
        # Background loading
        self._loader_pool = ThreadPoolExecutor(max_workers=worker_threads, thread_name_prefix="MLLoader")
        ...
        logger.info(f"🚀 Async Enhanced ML System initialized (cache: {max_cache_size}, workers: {worker_threads})")
        ...
        try:
            ...
            for horizon in prediction_horizons:
                ...
                for model_type in model_types:
                    model_path = f"{self.model_directory}/{symbol}_{horizon}_{model_type}.pkl"
                    
                    if os.path.exists(model_path):
                        try:
                            # This is the blocking operation, but it's in background thread
                            ...
                            logger.debug(f"✅ Loaded {symbol}_{horizon}_{model_type}")
                        except Exception as e:
                            logger.warning(f"Failed to load {symbol}_{horizon}_{model_type}: {e}")
            ...
            logger.info(f"🎯 Background loaded models for {symbol} ({len(models_loaded)} horizons)")
    ...
    def request_predictions_async(self, symbol: str, data: Any) -> str:
        ...
        self._symbol_frequency[symbol] = self._symbol_frequency.get(symbol, 0) + 1
        ...
        with self._cache_lock:
            if symbol in self._model_cache:
                ...
                try:
                    predictions = self._predict_with_cached_models(symbol, data)
                ...
                if not self._is_loading.get(symbol, False):
                    try:
                        ...
                        logger.warning(f"Model loading queue full, skipping {symbol}")
    ...
    def get_predictions_result(self, request_id: str) -> Optional[Dict[str, Any]]:
        """Get prediction results by request_id"""
        with self._prediction_lock:
            return self._prediction_results.get(request_id)
    
    def _predict_with_cached_models(self, symbol: str, data: Any) -> Dict[str, Any]:
        ...
        for model_key, horizon_models in models.items():
            horizon = model_key.split('_')[-1]  # Extract horizon (1d, 3d, etc.)
            ...
            for model_type, model_info in horizon_models.items():
                try:
                    ...
                    logger.warning(f"Prediction error {symbol} {model_type}: {e}")
            
            if model_predictions:
                # Simple ensemble average
                avg_prediction = sum(p['prediction'] for p in model_predictions) / len(model_predictions)
                avg_confidence = sum(p['confidence'] for p in model_predictions) / len(model_predictions)
                
                predictions[horizon] = {
                    'prediction': avg_prediction,
                    'confidence': avg_confidence,
                    'models_used': [p['type'] for p in model_predictions]
                }
    ...
    def preload_popular_symbols(self, symbols: list, max_preload: int = 50):
        ...
        # Sort by frequency
        sorted_symbols = sorted(symbols, key=lambda s: self._symbol_frequency.get(s, 0), reverse=True)
        
        for symbol in sorted_symbols[:max_preload]:
            if symbol not in self._model_cache and not self._is_loading.get(symbol, False):
                try:
                    self._loading_queue.put_nowait(symbol)
                    logger.info(f"🎯 Queued preload for popular symbol: {symbol}")
                except queue.Full:
                    break
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        with self._cache_lock:
            return {
                ...
                'loading_symbols': [s for s, loading in self._is_loading.items() if loading],
    ...
    def cleanup_old_results(self, max_age_seconds: int = 300):
        ...
        if to_remove:
            logger.debug(f"🧹 Cleaned up {len(to_remove)} old prediction results")
