...
# Üretimde .env kullanılmıyor; yalnızca mevcut ortam değişkenlerine
# güveniyoruz.
# .env yükleme mantığı bilinçli olarak kaldırıldı (sadelik ve deterministik
# davranış için).
...
class Config:
    ...
    if not SECRET_KEY:
        ...
        # Avoid printing secrets in production logs; provide minimal
        # guidance instead
        print(
            "⚠️ FLASK_SECRET_KEY is not set. A temporary key was "
    ...
    REMEMBER_COOKIE_HTTPONLY = (
        os.getenv('REMEMBER_COOKIE_HTTPONLY', 'True').lower() == 'true'
    )
    ...
    if not DATABASE_URL:
        # ✅ FIX: Try to construct DATABASE_URL from secret file and other
        # env vars
        ...
        try:
            ...
            if os.path.exists(password_file):
                ...
                logger.info(
                    f"✅ DATABASE_URL constructed from secret file "
                    f"(port {db_port})"
                )
            else:
                logger.error(
                    "DATABASE_URL environment variable is required and "
            ...
            logger.error(
                f"Failed to construct DATABASE_URL from secret file: {e}"
            )
            raise ValueError(
                "DATABASE_URL must be configured for PostgreSQL connection"
            )
    ...
    EMAIL_VERIFICATION_SECRET = os.getenv(
        'EMAIL_VERIFICATION_SECRET', SECRET_KEY
    )
    ...
    AUTO_CREATE_STOCKS = (
        os.getenv('AUTO_CREATE_STOCKS', 'True').lower() == 'true'
    )
    VALIDATE_SYMBOLS = (
        os.getenv('VALIDATE_SYMBOLS', 'False').lower() == 'true'
    )
    ALLOWED_SYMBOL_PATTERN = os.getenv(
        'ALLOWED_SYMBOL_PATTERN', r'^[A-Z0-9]{3,6}$'
    )

    # Yahoo Finance fetch settings
    YF_MAX_RETRIES = int(os.getenv('YF_MAX_RETRIES', '3'))
    YF_BACKOFF_BASE_SECONDS = float(
        os.getenv('YF_BACKOFF_BASE_SECONDS', '1.5')
    )
    YF_USER_AGENTS = [
        ua.strip() for ua in os.getenv('YF_USER_AGENTS', '').split('|')
        if ua.strip()
    ]

    # Enhanced Yahoo Finance with curl_cffi settings
    ENABLE_YAHOO_FALLBACK = (
        os.getenv('ENABLE_YAHOO_FALLBACK', 'True').lower() == 'true'
    )
    YF_ENHANCED_ENABLED = (
        os.getenv('YF_ENHANCED_ENABLED', 'True').lower() == 'true'
    )
    ...
    GUNICORN_WORKER_CLASS = os.getenv(
    ...
    NEWS_SOURCES = [
        source.strip() for source in os.getenv('NEWS_SOURCES', '').split(',')
        if source.strip()
    ]
    ...
    RUNNING_FLAG_HEARTBEAT_SECONDS = int(
        os.getenv('RUNNING_FLAG_HEARTBEAT_SECONDS', '20')
    )
    AUTO_START_PIPELINE = (
        os.getenv('AUTO_START_PIPELINE', 'True').lower() == 'true'
    )

    # Machine Learning Cache Paths
    # ✅ FIX: Use .cache/huggingface (actual path) instead of
    # cache/huggingface
    TRANSFORMERS_CACHE = os.getenv(
        'TRANSFORMERS_CACHE', '/opt/bist-pattern/.cache/huggingface'
    )
    HF_HOME = os.getenv(
        'HF_HOME', '/opt/bist-pattern/.cache/huggingface'
    )
    ...
    if not INTERNAL_API_TOKEN:
        logger.warning(
            "⚠️ INTERNAL_API_TOKEN not set - internal API will be "
            "disabled"
        )

    # Localhost access control (default: disabled for security)
    INTERNAL_ALLOW_LOCALHOST = (
        os.getenv('INTERNAL_ALLOW_LOCALHOST', 'False').lower() == 'true'
    )
    ...
    DISABLE_INTERNAL_SCHEDULER = (
        os.getenv('DISABLE_INTERNAL_SCHEDULER', 'False').lower() == 'true'
    )
    ...
    # Collection behavior
    # ALL | PRIORITY | DB_ACTIVE
    COLLECTION_SCOPE = os.getenv('COLLECTION_SCOPE', 'ALL')
    ...
    BROADCAST_PROGRESS = (
        os.getenv('BROADCAST_PROGRESS', 'True').lower() == 'true'
    )
    MIN_HISTORY_DAYS = int(os.getenv('MIN_HISTORY_DAYS', '365'))

    # Model/Detection tuning (env via systemd override)
    ENABLE_ENHANCED_ML = os.getenv(
        'ENABLE_ENHANCED_ML', 'True'
    ).lower() in ('1', 'true', 'yes')
    ENABLE_BASIC_ML = os.getenv(
        'ENABLE_BASIC_ML', 'True'
    ).lower() in ('1', 'true', 'yes')
    ENABLE_FINGPT = os.getenv(
        'ENABLE_FINGPT', 'True'
    ).lower() in ('1', 'true', 'yes')
    ...
    # default lookback
    PATTERN_DATA_DAYS = int(os.getenv('PATTERN_DATA_DAYS', '365'))

    # Scheduler/loop timing
    FULL_CYCLE_SLEEP_SECONDS = int(
        float(os.getenv('FULL_CYCLE_SLEEP_SECONDS', '300'))
    )
    ...
    YOLO_MODEL_PATH = os.getenv(
        'YOLO_MODEL_PATH', '/opt/bist-pattern/yolo/patterns_all_v1.pt'
    )
    ML_MODEL_PATH = os.getenv(
        'ML_MODEL_PATH', '/opt/bist-pattern/.cache/enhanced_ml_models'
    )
    CATBOOST_TRAIN_DIR = os.getenv(
